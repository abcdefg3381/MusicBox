...
public class Composer2 {
	...
	class NoteWithDegree extends AbstractMusicNote {

		private int degree = 0;

		public NoteWithDegree(AbstractMusicNote amn) {
			super();
			this.setId(amn.getId());
			this.setPitch(amn.getPitch());
			this.setLength(amn.getLength());
		}
	...
	private List<NoteWithDegree> noteWithDegreeList;
	...
	public void compose() {
		...
		try {
			...
			// retrive full edge list
			List<Edge> edgeList = edgeHandler.getAllEdges();
			noteWithDegreeList = new ArrayList<NoteWithDegree>();
			for (AbstractMusicNote amn : noteHandler.getAllAbstractMusicNotes()) {
				noteWithDegreeList.add(new NoteWithDegree(amn));
			}
			// set note degree
			for (Edge edge : edgeList) {
				for (NoteWithDegree note : noteWithDegreeList) {
					if (edge.getPreAMN().getId() == note.getId()) {
						note.setDegree(note.getDegree() + 1);
					}
				}
			}
			...
			for (int i = 0; i < 100; i++) {
				// 2. lookup/retrieve
				// 3. new note
				newNote = generateNextNote(oldID);
				if (newNote == null) {
	...
	private AbstractMusicNote generateNextNote(int oldID) {
	...
	private AbstractMusicNote randomNote(EdgeList edgeList) {
		...
		for (Edge edge : edgeList.getEdgeList()) {
			for (NoteWithDegree noteWithDegree : noteWithDegreeList) {
				if (edge.getSucAMN().getId() == noteWithDegree.getId()) {
					edge.setWeight(noteWithDegree.getDegree());
					break;
				}
			}
		}
