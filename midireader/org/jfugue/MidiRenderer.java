...
public final class MidiRenderer extends ParserListenerAdapter {
	...
	public void noteEvent(Note note) {
		...
		if (note.isRest()) {
			...
			this.eventManager.addNoteEvent(note.getValue(), attackVelocity, decayVelocity, duration, !note.isEndOfTie(),
					!note.isStartOfTie());
		}
	}
	...
	public void tempoEvent(Tempo tempo) {
		double tempoInMsPerBeat = TimeFactor.convertBPMToMicrosecondsPerBeat(tempo.getTempo());
		double d1 = Math.floor(tempoInMsPerBeat / (256.0 * 256.0));
		double d2 = Math.floor(tempoInMsPerBeat - d1 * 65536.0) / 256.0;
		double d3 = Math.floor(tempoInMsPerBeat - d1 * 65536.0 - d2 * 256.0);
		this.eventManager.addMetaMessage(0x51, new byte[] { (byte) d1, (byte) d2, (byte) d3 });
	}
