...
public class Pattern {
	...
	public void offset(long offsetTime) {
		...
		for (String token : tokens) {
			if ((token.length() > 0) && (token.charAt(0) == '@')) {
				String timeNumberString = token.substring(1, token.length());
	 ...
	 * specified once in a repeated pattern. Example: If the pattern is
	 * "T0 A B", calling <code>repeat(4, 3)</code> will make the pattern
	 * "T0 A B A B A B A B".
	 * 
	 * @version 3.0
	 */
	 ...
	 * <code>repeat(4, 3, 5)</code> will make the pattern
	 * "T0 A B A B A B A B C".
	 * 
	 * @version 3.0
	 */
