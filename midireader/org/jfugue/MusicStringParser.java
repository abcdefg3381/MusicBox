...
public final class MusicStringParser extends Parser {
	...
	private static void verifyTokenParsing() {
		...
		try {
			...
			parser.verifyToken(
					"C5minw",
					Note.createCompoundVerifyString(Note.createVerifyString(60, 1.0),
							Note.createVerifyString(63, 1.0, false, true, false),
							Note.createVerifyString(67, 1.0, false, true, false)));
			parser.verifyToken(
					"Cmaj",
					Note.createCompoundVerifyString(Note.createVerifyString(36, 0.25),
	...
	private void computeNoteValue(NoteContext context) {
		// This method does a variety of calculations to get the actual value of
		// the note.
	...
	private void fireNoteEvents(NoteContext context) {
		...
		if (context.isFirstNote) {
			...
			fireNoteEvent(note);
		} else if (context.anotherNoteIsSequential) {
	...
	private void parseControllerElement(String s) throws JFugueException {
		...
		if (-1 != controlIndexInt) {
			...
			trace("Combined controller element: coarse-index = ", coarseIndex, ", coarse-value = ", coarseValue,
					"; fine-index = ", fineIndex, ", fine-value = ", fineValue);
	...
	private int parseNoteChord(String s, int slen, int index, NoteContext context) {
		...
		if (lengthOfChordString > 0) {
			...
			trace("Chord: chordLength=", lengthOfChordString, ", so chord is one of the following: [ 3=", possibleChord3,
					" 4=", possibleChord4, " 5=", possibleChord5, " 6=", possibleChord6, " 7=", possibleChord7, " 8=",
					possibleChord8, " ]");
		}
	...
	private String parseNoteConnector(String s, int slen, int index, NoteContext context) {
		...
		if ((index < slen) && ((s.charAt(index) == '+') || (s.charAt(index) == '_'))) {
			...
			trace("Sequential? ", context.anotherNoteIsSequential);
	...
	private int parseNoteDuration(String s, int slen, int index, NoteContext context) {
		...
		// Tempo is now in Beats Per Minute. Convert this to Pulses Per Quarter
		// (PPQ), then to
		// Pulses Per Whole (PPW), then multiply that by durationNumber for
		// WHQITXN notes
		double ppq = 60000000.0D / this.getTempo();
		...
		if ('[' == s.charAt(index)) {
			...
			while (keepAdvancingPointer) {
				try {
					...
					if ((numericDurationChar >= '0') && (numericDurationChar <= '9') || (numericDurationChar == '.')) // Decimal
																														// dot,
																														// not
																														// dotted
																														// duration
	...
	private void parseTempoElement(String s) throws JFugueException {
		...
		setTempo(tempoNumber); // Set the tempo locally so it can be used to
	 ...
	 * 
	 * As of JFugue 4.0, Tempo measures Beats Per Minute (in the past, it had
	 * represented pulses per quarter, i.e., microseconds per beat, which was a
	 * bit of a backwards way of looking at tempo
	 * 
	 * @param tempo
	 *            the tempo for the current song, in beats per minute.
	 */
