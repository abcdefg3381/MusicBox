 ...
 * controller is being used to manipulate the Score object, as without using
 * this extra Thread the GUI would effectively be tied-up until the Score had
 * finished playing (which in the case of a loop means it would never be untied,
 * rendering the GUI useless).
 */
public class PlayCycle extends Thread {
	...
	/**
	 * Creates a PlayThread, waits for the length of time the score will play
	 * for, then repeats (loops) while Play.cycleIsPlaying() == true.
	 */
	@Override
	public void run() {
		...
		while (Play.cycleIsPlaying()) {
			...
			new Thread(pt).start();
			// waits for a Score-length of time until starting to load the next
			// segment
