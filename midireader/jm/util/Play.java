...
public class Play implements JMC {
	...
	/**
	 * Playback an audio file jMusic audio playback via javaSound. This method
	 * requires the javax.sound packages in Java 1.3 or higher.
	 * 
	 * @param String
	...
	/**
	 * Playback an audio file jMusic audio playback via javaSound. jMusic
	 * currently supports playback of .au, .wav and .aif file formats. This
	 * method requires the javax.sound packages in Java 1.3 or higher. By
	 * default this method, when complete, will exit the application. To prevent
	 * this pass 'false' as the autoClose argument.
	 * 
	 * @param filename
	 *            The name of the file to be played.
	 * @param autoClose
	 *            A flag for exiting java after the file has played.
	 */
	public static void au(String fileName, boolean autoClose) {
		...
		jm.audio.RTMixer mixer = new jm.audio.RTMixer(lineArray);// , 4096,
																	// si.getSampleRate(),
																	// si.getChannels(),
																	// 0.01);
		mixer.begin();
		System.out.println("---------- Playing '" + fileName + "'... Sample rate = " + afr.getSampleRate() + " Channels = "
				+ afr.getChannels() + " ----------");
		if (autoClose) {
			...
			try {
				...
				Thread.sleep((int) ((double) audioFile.length() / byteSize / afr.getSampleRate() / afr.getChannels() * 1000.0) + 1000);
	 ...
	 * @param inst
	 *            An instrument to play the part with
	 */
	 ...
	 *            The part to be played.
	 * @param insts
	 *            An array of instruments to play the part with
	 */
	 ...
	 * @param inst
	 *            An instrument to play the phrase with
	 */
	 ...
	 *            The phrase to be played.
	 * @param insts
	 *            An array of instruments to play the phrase with
	 */
	 ...
	 * @param inst
	 *            An instrument to play the score with
	 */
	 ...
	 *            The score to be played.
	 * @param insts
	 *            An array of instrument to play the score with
	 */
	public static void audio(Score score, Instrument[] insts) {
		...
		for (Instrument inst : insts) {
			inst.setOutput(Instrument.REALTIME);
		}
	...
	/**
	 * Playback an audio file using Java Applet audioclip playback. A audioClip
	 * limitation is that the file must be small enough to fit into RAM. This
	 * method is compatibl with Java 1.1 and higher.
	 * 
	 * @param String
	 *            The name of the file to be played.
	 */
	 ...
	 * 
	 * @param fileName
	 *            The name of the file to play back.
	 */
	 ...
	 *            The note to be played
	 * @param exit
	 *            Crash program after playabck? true or false
	 */
	...
	/**
	 * Playback the jMusic score JavaSound MIDI using the default value of true
	 * for 'exit' - See Play.midi(Score,boolean)
	 * 
	 * @param Score
	 *            : The score to be played.
	 */
	...
	/**
	 * Playback the jMusic score JavaSound MIDI. This method exits the
	 * application on completion. To avoid this exit call, pass false as the
	 * second argument.
	 * 
	 * @param Score
	 *            : The score to be played.
	...
	public static void midi(Score s, boolean exit) {
		...
		midiWait(s, exit);
		/*
		 * try { waitCycle(defensiveCopy); if (exit)System.exit(0); // horrid
		 * but less confusing for beginners }catch (Exception e) {
		 * System.err.println("jMusic MIDI Playback Error:" + e); return; }
	 ...
	 * 
	 * @param Note
	 *            The note to be played. See midiCycle(Score s)
	 */
	 ...
	 * 
	 * @param Part
	 *            The Part to be played. See midiCycle(Score s)
	 */
	 ...
	 * 
	 * @param Phrase
	 *            The Phrase to be played. See midiCycle(Score s)
	 */
	 ...
	 * reference is altered, that alteration will be heard on the following
	 * cycle. Score should be a minimum of 1 beat long. NB: It takes a small
	 * amount of time to load the Score. Due to the use of threads and a timer,
	 * no delay is heard between loops (i.e. the score is prepared just in time
	 * to be played). However, this means that alterations made to the Score
	 * object near the end of the segment (ie final beats) may not be heard for
	 * an extra loop. Putting this another way, if a 1-bar Score is being
	 * looped, the next bar may be pre-loaded as the last beat is being played.
	 * This means that any alterations made after this point will not appear in
	 * the very next loop.
	 */
	...
	/**
	 * Used by infinite cycle players (eg midiCycle()) to stop the playback
	 * loop.
	 */
	...
	/**
	 * Thread.sleeps for a period of 1 score play length (i.e. the time it would
	 * take for the specified score to play). Can be used in conjunction with
	 * midiCycle() if the score requires a re-compute just before being
	 * replayed. (i.e. sleeps for one loop) Should be placed immediately after
	 * the Play.midiCycle() command to ensure that the time waited is for the
	 * currently playing score, and not subject to any changes made to the score
	 * since.
	 * 
	 * @param Score
	 *            The score used for timing the sleep.
	 */
