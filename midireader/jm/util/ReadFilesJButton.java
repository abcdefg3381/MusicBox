 ...
 * 
 * <P>
 * After each successful import of a Score, any registered ReadListeners are
 * notified and can update and use the score read. The listeners are guaranteed
 * to be notified in a LILO (Last In Last Out) order. As an example, if you
 * wanted a Score quantised and then analysed, you would the quantising
...
public class ReadFilesJButton extends JButton {
	...
	/**
	 * Constant defining the state of a ReadFileJButton that reads in a folder
	 * of jm and/or midi files.
	 */
	 ...
	 * 
	 */
	private static final long serialVersionUID = -27737209642489255L;

	/**
	 * Constant defining the state of a ReadFileJButton that reads in a single
	 * jm or midi file.
	 */
	public static final Mode SINGLE_FILE_MODE = new Mode("Single File");

	/**
	 * JFileChooser that generates the dialogs for selecting the music files to
	 * import
	 */
	 ...
	 * default mode of {@link #MULTIPLE_FILES_MODE} which allows for a selection
	 * of files to be imported
	 * 
	 * @param owner
	 *            Component which is the owner of this button. Access to this
	 *            Component will be suspended when the user is selecting a music
	 *            file and when error messages are displayed.
	 */
	 ...
	 *            file and when error messages are displayed.
	 * @param readMode
	 *            Mode specified the file/folder selection mode
	 */
					...
					public void run() {
						...
						if (mode == SINGLE_FILE_MODE) {
							...
							if (file.isDirectory()) {
								 ...
								 * 
								 * processFiles(file.listFiles( new
								 * ReadFilenameFilter()));
								 */
								String[] filenames = file.list(new ReadFilenameFilter());
								for (String filename : filenames) {
									processFile(new File(file.getAbsolutePath(), filename));
								}
							}
						}
	 ...
	 * 
	 * <P>
	 * The task of file reading and conversion is delegated to {@link
	 * Read.midiOrJmWithSwingMessaging(File, Component)}.
	 * 
	 * @param file
	 *            File to convert to a score
	 * 
	 * @see #processFiles
	 */
	...
	public void setMode(Mode mode) {
		...
		if (mode == SINGLE_FILE_MODE) {
			...
			setText("Read Folder");
			chooser.setDialogTitle("Select a folder of MIDI or jMusic files to " + "import");
			chooser.setMultiSelectionEnabled(false);
