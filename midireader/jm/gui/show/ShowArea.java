...
public class ShowArea extends Canvas {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8273976165998146831L;
	...
	private int[] noteOffset = { 0, 0, noteHeight, noteHeight, noteHeight * 2, noteHeight * 3, noteHeight * 3, noteHeight * 4,
			noteHeight * 4, noteHeight * 5, noteHeight * 5, noteHeight * 6 };
	// attributes
	private int oldX;
	...
	public void paint(Graphics offScreenGraphics) {
		// Image offScreenImage = this.createImage(this.getSize().width,
		// this.areaHeight);
			...
			while (enum2.hasMoreElements()) {
				...
				while (enum3.hasMoreElements()) {
					...
					if ((currNote <= 127) && (currNote >= 0)) {
						...
						int y = ((10 - currNote / 12) * octavePixelheight + (ePos)) - noteOffset[currNote % 12];
						int x = (int) (Math.round(aNote.getDuration() * beatWidth)); // 480
																						// ppq
																						// note
						int xRV = (int) (Math.round(aNote.getRhythmValue() * beatWidth)); // 480
																							// ppq
																							// note
						...
						offScreenGraphics.setColor(Color.getHSBColor(theColours[i % maxColours][0],
								theColours[i % maxColours][1], (float) (0.7 - (aNote.getDynamic() * 0.004))));
						// draw note inside
						if (aNote.getPitchType() == Note.MIDI_PITCH) {
							offScreenGraphics.fillRect(oldX, y - noteHeight + thinNote, x, noteHeight * 2 - 2 * thinNote);
						} else { // draw frequency derrived note
							int heightOffset = 7;
							for (int j = oldX; j < oldX + x - 4; j += 4) {
								offScreenGraphics.drawLine(j, y - noteHeight + heightOffset, j + 2, y - noteHeight
										+ heightOffset - 3);
								offScreenGraphics.drawLine(j + 2, y - noteHeight + heightOffset - 3, j + 4, y - noteHeight
										+ heightOffset);
							}
						}
						...
						offScreenGraphics.drawRect(oldX, y - noteHeight + thinNote, xRV, noteHeight * 2 - 2 * thinNote);
						// add a sharp if required
						if ((currNote % 12) == 1 || (currNote % 12) == 3 || (currNote % 12) == 6 || (currNote % 12) == 8
								|| (currNote % 12) == 10) {
							offScreenGraphics.setColor(Color.getHSBColor(theColours[i % maxColours][0], theColours[i
									% maxColours][1], (float) (0.3)));
							offScreenGraphics.drawString("#", oldX - 7, y + 5);
						}
					}
					oldXBeat += aNote.getRhythmValue();
					oldX = (int) (Math.round(oldXBeat * beatWidth));
					rectRight = oldX - rectLeft; // update value for phrase
													// rectangle
				}
				// draw the phrase rectangle
				// offScreenGraphics.setColor(Color.lightGray);
				offScreenGraphics.setColor(Color.getHSBColor(theColours[i % maxColours][0], theColours[i % maxColours][1],
						(float) (0.9)));
				offScreenGraphics.drawRect(rectLeft - 1, rectTop - noteHeight - 1, rectRight + 1, rectBot - rectTop
						+ noteHeight * 2 + 2);
			}
	...
	private void reInit() {
		...
		noteOffset =
				new int[] { 0, 0, noteHeight, noteHeight, noteHeight * 2, noteHeight * 3, noteHeight * 3, noteHeight * 4,
						noteHeight * 4, noteHeight * 5, noteHeight * 5, noteHeight * 6 };
