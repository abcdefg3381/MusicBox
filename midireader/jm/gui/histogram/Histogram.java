...
/**
 * Display a visual histogram of values from a jMusic score. The display can
 * show one of several features from the jMusic notes in the specified score.
...
public class Histogram extends Component implements JMC {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4985478031018416960L;
	...
	/**
	 * Histogram constructor specifying the score and type of data to be
	 * displayed.
	 * 
	 * @param score
	 *            the score to be displayed
	 * @param type
	 *            the note attribute to be displayed, 0 = pitch etc.
	 */
	 ...
	 *            the note attribute to be displayed, 0 = pitch etc.
	 * @param xPos
	 *            the horizonal position for the window to be displayed
	 * @param yPos
	 *            the vertical position for the window to be displayed
	 */
					...
					if (note.getPitchType() == Note.MIDI_PITCH) {
						if (note.getPitch() != REST) {
							...
							if (pitchValues[note.getPitch()] > maxPitchValue) {
								maxPitchValue = pitchValues[note.getPitch()];
							}
							// rhythm
							...
							if (dynamicValues[note.getDynamic()] > maxDynamicValue) {
								maxDynamicValue = dynamicValues[note.getDynamic()];
							}
							// pan
							panValues[(int) (note.getPan() * 100)]++;
							if (panValues[(int) (note.getPan() * 100)] > maxPanValue) {
								maxPanValue = panValues[(int) (note.getPan() * 100)];
							}
						}
					}
				}
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < 5; i++) {
			...
			g.drawString(lString, this.getSize().width / 5 * i + lableSpace - fm.stringWidth(lString) / 2,
					lableSpace - fm.getAscent() / 2);
			g.setColor(Color.gray);
			g.drawLine(this.getSize().width / 5 * i + lableSpace, lableSpace, this.getSize().width / 5 * i + lableSpace,
					this.getSize().height);
		}
	...
	private void paintDynamics(Graphics g) {
		...
		for (int i = 1; i < 127; i++) {
			...
			g.fillRect(
					lableSpace,
					i * barWidth + lableSpace,
					(int) ((double) dynamicValues[i] / (double) maxDynamicValue * ((double) this.getSize().width - lableSpace)),
					barWidth - 1);
			if (i % 10 == 0) {
				g.setColor(Color.red);
	...
	private void paintPans(Graphics g) {
		...
		for (int i = 1; i < 100; i++) {
			...
			g.fillRect(lableSpace, i * barWidth + lableSpace,
					(int) ((double) panValues[i] / (double) maxPanValue * ((double) this.getSize().width - lableSpace)),
					barWidth - 1);
			if (i % 10 == 0 && i != 50) {
				g.setColor(Color.red);
	...
	private void paintPitches(Graphics g) {
		...
		for (int i = 0; i < 127; i++) {
			...
			g.fillRect(lableSpace, i * barWidth + lableSpace,
					(int) ((double) pitchValues[i] / (double) maxPitchValue * ((double) this.getSize().width - lableSpace)),
					barWidth - 1);
			if (i % 12 == 0) {
				g.setColor(Color.red);
	...
	private void paintRhythms(Graphics g) {
		...
		for (int i = 1; i < 65; i++) {
			...
			g.fillRect(lableSpace, i * barWidth * 2 + lableSpace,
					(int) ((double) rhythmValues[i] / (double) maxRhythmValue * ((double) this.getSize().width - lableSpace)),
					barWidth * 2 - 1);
			if (i % 8 == 0) {
				g.setColor(Color.red);
	...
	/**
	 * Save the histogram data to a tab delimited text file with a file name to
	 * be specified by a dialog box.
	 */
	...
	/**
	 * Save the histogram data to a tab delimited text file of the specified
	 * name
	 * 
	 * @param fileName
	...
	public void saveDataAs(String fileName) {
		try {
			...
			String headerText =
					"Pitch value" + String.valueOf("\t") + "Pitch data" + String.valueOf("\t") + "Rhythm value"
							+ String.valueOf("\t") + "Rhythm data" + String.valueOf("\t") + "Dynamic value"
							+ String.valueOf("\t") + "Dynamic data" + String.valueOf("\t") + "Pan value" + String.valueOf("\t")
							+ "Pan data" + String.valueOf("\n");
			...
			for (int i = 0; i < pitchValues.length; i++) {
				...
				if (i < rhythmValues.length) {
					data +=
							String.valueOf("\t") + String.valueOf(i * 0.125) + String.valueOf("\t")
									+ String.valueOf(rhythmValues[i]);
				}
				if (i < dynamicValues.length) {
					data += String.valueOf("\t") + String.valueOf(i) + String.valueOf("\t") + String.valueOf(dynamicValues[i]);
				}
				if (i < panValues.length) {
					data +=
							String.valueOf("\t") + String.valueOf(i / 100.0) + String.valueOf("\t")
									+ String.valueOf(panValues[i]);
				}
				data += String.valueOf("\n");
