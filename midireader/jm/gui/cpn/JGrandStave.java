...
public class JGrandStave extends JPanel implements JMC {
	class RedFilter {
		...
		RedFilter(Image image) {
			this.image = image;
			redImage = createImage(new FilteredImageSource(image.getSource(), filter));
		}
	}

	public final static int[] keys = { 11, 6, 1, 8, 3, 10, 5, 0, 7, 2, 9, 4, 11, 6, 1 };
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1203349561695360695L;
	...
	protected int[] notePosOffset = { 24, 24, 20, 20, 16, 12, 12, 8, 8, 4, 4, 0 }; // chromatic
	...
	public int staveSpaceHeight = 8, rightMargin = 20, beatWidth = 43, staveWidth = beatWidth * 15, imageHeightOffset = 28,
			clefWidth = 38, timeSigWidth = 5, keySigWidth = 5;
	...
	protected Image trebleClef, bassClef, one, two, three, four, five, six, seven, eight, nine;
	...
	public JGrandStave(Phrase phrase) {
		...
		// change 'paper' colour
		this.setBackground(Color.getHSBColor((float) 0.14, (float) 0.09, (float) 1.0)); // .17,
																						// .1,
																						// 1
		...
		try {
			...
			// tracker.waitForID(0); // still incomplete media tracker
			// implementation
		} catch (Exception e) {
		...
		crotchetDown.redImage = createImage(new FilteredImageSource(crotchetDown.image.getSource(), filter));
		crotchetUp.redImage = createImage(new FilteredImageSource(crotchetUp.image.getSource(), filter));
		quaverDown.redImage = createImage(new FilteredImageSource(quaverDown.image.getSource(), filter));
		quaverUp.redImage = createImage(new FilteredImageSource(quaverUp.image.getSource(), filter));
		semiquaverDown.redImage = createImage(new FilteredImageSource(semiquaverDown.image.getSource(), filter));
		semiquaverUp.redImage = createImage(new FilteredImageSource(semiquaverUp.image.getSource(), filter));
		minimDown.redImage = createImage(new FilteredImageSource(minimDown.image.getSource(), filter));
		minimUp.redImage = createImage(new FilteredImageSource(minimUp.image.getSource(), filter));
		semibreve.redImage = createImage(new FilteredImageSource(semibreve.image.getSource(), filter));
		dot.redImage = createImage(new FilteredImageSource(dot.image.getSource(), filter));
		semiquaverRest.redImage = createImage(new FilteredImageSource(semiquaverRest.image.getSource(), filter));
		quaverRest.redImage = createImage(new FilteredImageSource(quaverRest.image.getSource(), filter));
		crotchetRest.redImage = createImage(new FilteredImageSource(crotchetRest.image.getSource(), filter));
		minimRest.redImage = createImage(new FilteredImageSource(minimRest.image.getSource(), filter));
		sharp.redImage = createImage(new FilteredImageSource(sharp.image.getSource(), filter));
		flat.redImage = createImage(new FilteredImageSource(flat.image.getSource(), filter));
		natural.redImage = createImage(new FilteredImageSource(natural.image.getSource(), filter));
		delete.redImage = createImage(new FilteredImageSource(delete.image.getSource(), filter));
		 ...
		 * reimplemented once we shift completely to JDK1.2.
		 * 
		 * tracker.addImage(trebleClef, 0); tracker.addImage(bassClef, 1);
		 ...
		 * tracker.addImage(minimDown.image, 8); tracker.addImage(minimUp.image,
		 * 9); tracker.addImage(semibreve.image, 10);
		 ...
		 * tracker.addImage(sharp.image, 17); tracker.addImage(flat.image, 18);
		 * tracker.addImage(natural.image, 19); tracker.addImage(one, 20);
		 * tracker.addImage(two, 21); tracker.addImage(three, 22);
		 * tracker.addImage(four, 23); tracker.addImage(five, 24);
		 * tracker.addImage(six, 25); tracker.addImage(seven, 26);
		 * tracker.addImage(eight, 27); tracker.addImage(nine, 28);
		 ...
		 * tracker.addImage(sharp.redImage, 47); tracker.addImage(flat.redImage,
		 * 48); tracker.addImage(natural.redImage, 49); // tracker.addImage(one,
		 * 20); // tracker.addImage(two, 21); // tracker.addImage(three, 22); //
		 * tracker.addImage(four, 23); // tracker.addImage(five, 24); //
		 * tracker.addImage(six, 25); // tracker.addImage(seven, 26); //
		 * tracker.addImage(eight, 27); // tracker.addImage(nine, 28);
		 * tracker.addImage(delete.redImage, 50); tracker.addImage(tie.image,
		 * 51); tracker.addImage(tie.redImage, 52);
		 * 
		 * try { System.out.println("Loading images..."); tracker.waitForAll();
	...
	protected void chooseImage(int pitch, double rhythmValue, int upPitch1, int downPitch, int upPitch2) {
		...
		if (pitch == REST) { // pick a rest
			...
			isNote = true;
			if ((pitch < upPitch1 && pitch >= downPitch) || pitch < upPitch2) { // stems
																				// down
				if (rhythmValue <= 0.0) {
					currImage = delete;
					currBeatWidth = (int) (beatWidth * 0.5);
	...
	public void displayImage(final Graphics g, final RedFilter image, int xCoord, int yCoord) {
		if (isNormalColor) {
	...
	private void drawNote(int notePitchNum, final double rhythmValue, int pitchTempPos, final int keyAccidentals) {
		...
		if ((beatCounter % 1.0) == 0.0) {
			...
			while (lastChordDisplayed < currentBeat) {
				...
				g.drawString(chordStrings[firstChords[lastChordDisplayed]],
						(int) (totalBeatWidth - ((totalBeatWidth - lastPosition) * (remaining / (double) total))), 20);
				...
				g.drawString(string, (int) (totalBeatWidth - ((totalBeatWidth - lastPosition) * (remaining / (double) total))),
						40);
			}
		...
		drawNote2(notePitchNum, rhythmValue - excessRhythmValue, pitchTempPos, keyAccidentals);
		if (requiresMoreThanOneImage) {
			drawNote(notePitchNum, excessRhythmValue, pitchTempPos, keyAccidentals);
	...
	/*
	 * 
	 * g.drawImage(createImage(new FilteredImageSource(image.getSource(), new
	 * RGBImageFilter() { { canFilterIndexColorModel = true; }
	 * 
	 * public int filterRGB(int x, int y, int rgb) { return (rgb & 0xffff0000);
	 * } })), xCoord, yCoord, this);
	 * 
	 * g.drawIma int w = image.getWidth(this); int h = image.getHeight(this);
	 * 
	 * 
	 * int[] pixels = new int[w * h]; PixelGrabber pg = new PixelGrabber(image,
	 * 0, 0, w, h, pixels, 0, w); try { pg.grabPixels(); } catch
	 * (InterruptedException e) {
	 * System.err.println("interrupted waiting for pixels!"); return; }
	 * System.out.println("Status: " + pg.getStatus());
	 * System.out.println("ImageObserver.ABORT : " + ImageObserver.ABORT); if
	 * ((pg.getStatus() & ImageObserver.ABORT) != 0) {
	 * System.err.println("image fetch aborted or errored"); return; } for(int i
	 * = 0; i < pixels.length; i++){ System.out.println(pixels[i]); }
	 * 
	 * 
	 * IndexColorModel colorModel = new IndexColorModel(8, pixels.length,
	 * pixels, 0, DataBuffer.TYPE_BYTE, null); byte[] reds = new
	 * byte[pixels.length]; colorModel.getReds(reds); byte[] alphas = new
	 * byte[pixels.length]; colorModel.getAlphas(alphas);
	 * 
	 * IndexColorModel redModel = new IndexColorModel(8, pixels.length, reds,
	 * new byte[pixels.length], new byte[pixels.length], alphas); BufferedImage
	 * bufImage = new BufferedImage(w, h, BufferedImage.TYPE_BYTE_INDEXED,
	 * redModel); g.drawImage(bufImage, xCoord, yCoord, this); } }
	 */

	private void drawNote2(int notePitchNum, final double rhythmValue, int pitchTempPos, final int keyAccidentals) {

		// accidental?
		if (((notePitchNum % 12) == 1 || (notePitchNum % 12) == 3 || (notePitchNum % 12) == 6 || (notePitchNum % 12) == 8 || (notePitchNum % 12) == 10)
				&& notePitchNum != REST && rhythmValue != 0.0) {
			if (keySignature > -1) {
				...
				previouslyChromatic.addElement(new Integer(notePitchNum - 1)); // enter
																				// the
																				// note
																				// made
																				// sharp
				...
				pitchTempPos -= 4; // to show the note a semitone higher for
									// flats
				if (!firstAccidentalDisplayed) {
				...
				notePitchNum++; // assume it is a semitone higher for legerlines
								// etc...
			...
			for (int j = 0; j < size; j++) {
				...
				if (temp.intValue() == notePitchNum && notePitchNum != REST && rhythmValue != 0.0) {
					...
					// g.drawImage(natural, totalBeatWidth - 7, pitchTempPos,
					// this);
		...
		if (dottedNote) {
			...
			for (int l = 0; l < lineNotes.length; l++) {
				if (lineNotes[l] + 12 == notePitchNum || lineNotes[l] + 36 == notePitchNum || lineNotes[l] + 60 == notePitchNum
						|| lineNotes[l] + 84 == notePitchNum || lineNotes[l] + 108 == notePitchNum || notePitchNum == REST) {
					displayImage(g, dot, totalBeatWidth + 1, pitchTempPos - 4);
					// g.drawImage(dot, totalBeatWidth + 1, pitchTempPos - 4,
					// this);
					dotFlag = false;
					l = lineNotes.length;
				}
		...
		if ((isTied || extraImagesUsed) && isNote && !isFirstNoteInTie) {
			...
			if (isUp) {
				g.drawImage(tieImage, savedBeatWidth - 3 + 9, yPosition + 17 + tieImage.getHeight(this),
						savedBeatWidth2 + 19 - 9, yPosition + 17, 0, 0, tieImage.getWidth(this), tieImage.getHeight(this),

						this);
			} else {
				g.drawImage(tieImage, savedBeatWidth - 3 + 9, yPosition - 20, savedBeatWidth2 + 19 - 9, yPosition - 20
						+ tieImage.getHeight(this), 0, 0, tieImage.getWidth(this), tieImage.getHeight(this), this);
			}
		}
	...
	/**
	 * returns the current major key for this Stave instance as a integer 0 is
	 * C, 1 is C#/Db major, 2 is D major, etc
	 */
	public int getTonic() {
		return tonic;
	}
	...
	public void paintComponent(Graphics graphics) {
		...
		if (keySignature > 0 && keySignature < 8) { // sharp
			for (int ks = 0; ks < keySignature; ks++) {
				...
				int keyAccidentalPosition =
						notePosOffset[sharps[ks] % 12] + bPos - 4 + ((5 - sharps[ks] / 12) * 24) + ((6 - sharps[ks] / 12) * 4);
				// draw sharp on treble
				g.drawImage(sharp.image, rightMargin + clefWidth + keyOffset, keyAccidentalPosition, this);
				// draw sharp on bass
				g.drawImage(sharp.image, rightMargin + clefWidth + keyOffset, keyAccidentalPosition + staveSpaceHeight * 7,
						this);
				// indent position
				keyOffset += 10;
			...
			if (keySignature < 0 && keySignature > -8) { // flat
				for (int ks = 0; ks < Math.abs(keySignature); ks++) {
					// claulate position
					int keyAccidentalPosition =
							notePosOffset[flats[ks] % 12] + bPos - 4 + ((5 - flats[ks] / 12) * 24) + ((6 - flats[ks] / 12) * 4);
					// draw flat
					g.drawImage(flat.image, rightMargin + clefWidth + keyOffset, keyAccidentalPosition, this);
					// draw flat on bass stave
					g.drawImage(flat.image, rightMargin + clefWidth + keyOffset, keyAccidentalPosition + staveSpaceHeight * 7,
							this);
					// indent position
					keyOffset += 10;
					// add note to accidental vector
		...
		firstChords = ChordAnalysis.getFirstPassChords(phrase, 1.0, tonic, scale);
		secondChords = ChordAnalysis.getSecondPassChords(phrase, 1.0, tonic, scale);
		...
		for (int i = 0; i < phrase.size(); i++) {
			...
			// position?
			int pitchTempPos;
			if (notePitchNum == REST || phrase.getNote(i).getRhythmValue() == 0.0) { // rest
																						// or
																						// delete
			...
			if (notePitchNum == REST || PhraseAnalysis.isScale(phrase.getNote(i), tonic, scale)) {
				isNormalColor = true;
			} else {
				isNormalColor = false;
			}
			...
			while (rvToEndOfBar < rhythmValue) {
				isTied = true;
				drawNote(notePitchNum, rvToEndOfBar, pitchTempPos, keyAccidentals);
		...
		for (int i = 0; i < 5; i++) {
			g.drawLine(totalBeatWidth, (bPos + imageHeightOffset - (2 * staveSpaceHeight)) + (i * staveSpaceHeight),
					totalBeatWidth + 50, (bPos + imageHeightOffset - (2 * staveSpaceHeight)) + (i * staveSpaceHeight));
		}
		for (int i = 6; i < 11; i++) {
			g.drawLine(totalBeatWidth, (bPos + imageHeightOffset - (2 * staveSpaceHeight)) + (i * staveSpaceHeight),
					totalBeatWidth + 50, (bPos + imageHeightOffset - (2 * staveSpaceHeight)) + (i * staveSpaceHeight));
	...
	/**
	 * Sets the current key signature for this Stave instance This effects the
	 * displayed key signature. 1 = F# etc. 0 is no key signature, + numbers for
	 * sharps, - numbers for flats
	 * 
	 * @param int
	 */
	public void setKeySignature(int key) {
		this.keySignature = ((key > 7) ? 7 : ((key < -7) ? -7 : key));
	...
	/**
	 * Sets the current metre for this Stave instance This effects the displayed
	 * time signature. 4.0 = 4/4 etc.
	 * 
	 * @param double
	 */
	...
	public void setTonic(int degree) {
		...
		if (scale == Scales.MAJOR_SCALE) {
			tonic = degree;
		} else if (scale == Scales.NATURAL_MINOR_SCALE) {
			tonic = degree;
			degree = (degree + 3) % 12;
		} else {
			return;
	...
	/**
	 * Called by stave action on mouseUp Can be overridden by extending classes
