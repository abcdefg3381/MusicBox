...
public class TrebleStave extends Stave implements JMC {
	...
	public static abstract class Style {
		...
		private static final class JMusic extends Style {
			...
			/**
			 * Key signature encoded as a signed accidental count. The following
			 * conditions apply:
			 * <ul>
			...
			void initialise(final int keySignature) {
				...
				if (keySignature > 0 && keySignature < 8) {
					for (int i = 0; i < keySignature; i++) {
						...
						for (int j = Note.MIN_PITCH; j <= Note.MAX_PITCH; j++) {
							if ((j % 12) == degree) {
								chromaticallyAffectedPitches.addElement(new Integer(j));
								keyAccidentals++;
							}
						}
					}
			...
			@Override
			Accidental selectAccidental(final int pitch, final double rhythmValue) {
				if (pitch == Note.REST || rhythmValue == 0.0) {
					return Accidental.NONE;
				}
				if ((pitch % 12) == 1 || (pitch % 12) == 3 || (pitch % 12) == 6 || (pitch % 12) == 8 || (pitch % 12) == 10) {
					if (keySignature > -1) {
						chromaticallyAffectedPitches.addElement(new Integer(pitch - 1));
					...
					for (int j = 0; j < size; j++) {
						temp = ((Integer) chromaticallyAffectedPitches.elementAt(j)).intValue();
						if (temp == pitch) {
							if (j > keyAccidentals - 1) {
								chromaticallyAffectedPitches.removeElementAt(j);
							}
		...
		private static final class Trad extends Style {
			...
			void initialise(final int keySignature) {
				...
				this.setBooleanArrayToFalse(accidentalRequiredByKeySignature);
				...
				for (int i = 0; i < Math.abs(keySignature); i++) {
					if (keySignature < 0) {
						accidentalRequiredByKeySignature[flatPitches[i] % 12] = true;
						accidentalRequiredByKeySignature[(flatPitches[i] - 1) % 12] = false;
					} else {
						accidentalRequiredByKeySignature[sharpPitches[i] % 12] = true;
						accidentalRequiredByKeySignature[(sharpPitches[i] + 1) % 12] = false;
					}
				}
			...
			Accidental selectAccidental(final int pitch, final double rhythmValue) {
				...
				if (accidentalRequiredByKeySignature[degree] ^ accidentalInEffect[accidentalPair]) {

					accidentalInEffect[accidentalPair] = !accidentalInEffect[accidentalPair];
		...
		/**
		 * Defines a style unique to jMusic, which displays an accidental in all
		 * situations where the status (sharp/flat/natural) of a note may be
		 * unclear.
		 * 
		...
		abstract Accidental selectAccidental(final int pitch, final double rhythmValue);
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6823471031897959697L;
	 ...
	 * 
	 * @param images
	 *            Images representing notes, rest and other stave elements to
	 *            use within the compenent
	 */
	...
	private void displayImage(final Graphics g, final Image image, int xCoord, int yCoord) {
		g.drawImage(image, xCoord, yCoord, this);
	}

	private void drawNote(int notePitchNum, final double rhythmValue, int pitchTempPos) {
		...
		drawNote2(notePitchNum, rhythmValue - excessRhythmValue, pitchTempPos);
		if (requiresMoreThanOneImage) {
			drawNote(notePitchNum, excessRhythmValue, pitchTempPos);
	...
	private void drawNote2(int pitch, final double rhythmValue, int yCoordinate) {

		// draw accidental

		if (pitch != Note.REST && rhythmValue != 0.0) {
			Accidental accidental = style.selectAccidental(pitch, rhythmValue);
			if (accidental == Accidental.SHARP) {
				...
				yCoordinate -= 4; // to show the note a semitone higher for
									// flats
				if (!firstAccidentalDisplayed) {
		...
		if (dottedNote) {
			...
			for (int l = 0; l < lineNotes.length; l++) {
				if (lineNotes[l] + 12 == pitch || lineNotes[l] + 36 == pitch || lineNotes[l] + 60 == pitch
						|| lineNotes[l] + 84 == pitch || lineNotes[l] + 108 == pitch || pitch == REST) {
		...
		if ((isTied || extraImagesUsed) && isNote && !isFirstNoteInTie) {
			...
			if (isUp) {
				g.drawImage(tieUnder, savedBeatWidth - 3 + 9, yPosition + 17, savedBeatWidth2 + 19 - 9, yPosition + 17
						+ tieUnder.getHeight(this), 0, 0, tieUnder.getWidth(this), tieUnder.getHeight(this),

				this);
			} else {
				g.drawImage(tieOver, savedBeatWidth - 3 + 9, yPosition - 20, savedBeatWidth2 + 19 - 9,
						yPosition - 20 + tieOver.getHeight(this), 0, 0, tieOver.getWidth(this), tieOver.getHeight(this), this);
			}
		}
	...
	public void paint(Graphics graphics) {
		...
		for (int i = 0; i < phrase.size(); i++) {
			...
			while (rvToEndOfBar < rhythmValue) {
				isTied = true;
				drawNote(notePitchNum, rvToEndOfBar, pitchTempPos);
		...
		// g.drawImage(bassClef, rightMargin + 7, bPos + staveSpaceHeight * 6,
		// this);
