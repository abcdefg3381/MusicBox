...
public class GrandStave extends Stave implements JMC {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2866983080189429622L;
	...
	public void paint(Graphics graphics) {
		...
		for (int i = 0; i < phrase.size(); i++) {
			...
			if (((notePitchNum % 12) == 1 || (notePitchNum % 12) == 3 || (notePitchNum % 12) == 6 || (notePitchNum % 12) == 8 || (notePitchNum % 12) == 10)
					&& notePitchNum != REST && phrase.getNote(i).getRhythmValue() != 0.0) {
				if (keySignature > -1) {
			...
			if (metre != 0.0) {
				if ((beatCounter % metre) == 0.0) {
					g.drawLine(totalBeatWidth, bPos + 12 - staveSpaceHeight * 7, totalBeatWidth, bPos + 44 + staveSpaceHeight
							* 13);
					// add bar numbers?
					if (barNumbers) {
						g.drawString("" + (int) (beatCounter / metre + 1 + phrase.getStartTime()), totalBeatWidth - 4,
								bPos - 50);
					}
					totalBeatWidth += 12;
				}
