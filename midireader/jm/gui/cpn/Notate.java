...
/**
 * This class displays a frame with a common practice notation display of the
 * score passed to it. The parameter and add data by text attributes only work
 * on the first stave. Some GPL changes for jMusic CPN Written by Al Christians
 * (achrist@easystreet.com). Contributed by Trillium Resources Corporation,
 * Oregon's leading provider of unvarnished software.
 * 
 * @author Andrew Brown
 */
...
public class Notate extends Frame implements ActionListener, WindowListener, JMC {
	...
	private static double getRhythmAdjustment(double beats, double beatIncrement) {
		...
		while ((Math.floor(increments + tolerance) > n) && (tolerance > 0.00000000000001)) {
			answer = tolerance;
	...
	private MenuItem keySig, open, openJmXml, openjm, delete, clear, newStave, close, timeSig, saveJmXml, saveJM, saveMidi,
			quit, trebleStave, bassStave, pianoStave, grandStave, automaticStave,

			setParameters, playAll, playMeasure, repeatAll, repeatMeasure, stopPlay, addNotes, adjustTiming, viewDetails,
			viewTitle, viewZoom, barNumbers;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == close) {
			...
			for (Stave element : staveArray) {
				if (element.getKeySignature() == 0) {
					element.setKeySignature(2);
					element.repaint();
				} else {
					element.setKeySignature(0);
					element.repaint();
				}
			}
		} else if (e.getSource() == timeSig) {
			for (Stave element : staveArray) {
				if (element.getMetre() == 0.0) {
					element.setMetre(4.0);
					element.repaint();
				} else {
					element.setMetre(0.0);
					element.repaint();
				}
			}
			...
			for (Stave element : staveArray) {
				element.deleteLastNote();
			}
		} else if (e.getSource() == clear) {
			for (Stave element : staveArray) {
				element.getPhrase().empty();
				element.repaint();
			}
		...
		/*
		 * else if(e.getSource() == insertMidiFile) { }
		 * 
		 * setCursor(new Cursor(Cursor.WAIT_CURSOR)); Phrase savePhrase =
		 * phrase; phrase = readMidiPhrase(); phrase.addNoteList(
		 * savePhrase.getNoteList(), true ); stave.setPhrase(phrase); for(int
		 * i=0; i<staveArray.length; i++) { staveArray[i].repaint(); }
		 * setCursor(new Cursor(Cursor.DEFAULT_CURSOR)); }
		 * 
		 * else if(e.getSource() == appendMidiFile) { }
		 * 
		 * setCursor(new Cursor(Cursor.WAIT_CURSOR)); phrase.addNoteList(
		 * readMidiPhrase().getNoteList(), true ); stave.setPhrase(phrase);
		 * stave.repaint(); setCursor(new Cursor(Cursor.DEFAULT_CURSOR)); }
		 */
		else if (e.getSource() == setParameters) { // only works on the top
													// stave
		...
		} else if (e.getSource() == adjustTiming) { // only works on the top
													// stave
			adjustTimeValues(staveArray[0].getPhrase());
			staveArray[0].repaint();
		} else if (e.getSource() == viewDetails) { // only works on the top
													// stave
			PhraseViewer phraseViewer = new PhraseViewer(this);
			phraseViewer.showPhrase(staveArray[0], staveArray[0].getPhrase(), 15, 15);
			...
			if (!zoomed) {
				...
				beforeZoom.empty();
				afterZoom.empty();
				zoomSelector.zoomIn(beforeZoom, staveArray[0].getPhrase(), afterZoom);
				if (beforeZoom.size() + afterZoom.size() > 0) {
					zoomed = true;
					viewZoom.setLabel("View complete phrase");
					repaint();
				}
			} else {
				CpnZoomScreen.zoomOut(beforeZoom, staveArray[0].getPhrase(), afterZoom);
				zoomed = false;
	...
	public void init() {
		 ...
		 * insertMidiFile.addActionListener(this); features.add(insertMidiFile);
		 * 
		 * appendMidiFile = new MenuItem("Append a MIDI file..." );
		 * appendMidiFile.addActionListener(this); features.add(appendMidiFile);
		 */
		...
		/*
		 * // check window size against screen Toolkit tk =
		 * Toolkit.getDefaultToolkit(); Dimension d = tk.getScreenSize();
		 * if(scroll.getSize().height > d.height) {
		 * System.out.println("Adjusting height"); scroll.setSize(new
		 * Dimension(this.width, d.height)); }
		 */
		this.show();
	}
	...
	private void makeAppropriateStaves() {
		...
		for (int i = 0; i < score.size(); i++) {
			...
			if (currentPhrase.getHighestPitch() < A6 && currentPhrase.getLowestPitch() > FS3) {
				tempStaveArray[i] = new TrebleStave();
			} else if (currentPhrase.getHighestPitch() < F4 && currentPhrase.getLowestPitch() > B1) {
				tempStaveArray[i] = new BassStave();
			} else if (currentPhrase.getHighestPitch() > A6 || currentPhrase.getLowestPitch() < B1) {
	...
	/*
	 * // store current phrase phrase = stave.getPhrase().copy(); int tempKey =
	 * stave.getKeySignature(); double tempTime = stave.getMetre(); boolean
	 * tempBarNumbers = stave.getBarNumbers(); // create new stave panel stave =
	 * new BassStave(); scoreBG.removeAll(); scoreBG.add(stave);
	 * scroll.setSize(width, stave.getSize().height + 20); this.pack(); //
	 * replace stave stave.setPhrase(phrase); stave.setKeySignature(tempKey);
	 * stave.setMetre(tempTime); stave.setBarNumbers(tempBarNumbers); }
	 */
	 ...
	 * stave.getKeySignature(); double tempTime = stave.getMetre(); boolean
	 * tempBarNumbers = stave.getBarNumbers(); // create new stave panel stave =
	 * new PianoStave(); scoreBG.removeAll(); scoreBG.add(stave);
	 * scroll.setSize(width, stave.getSize().height + 20); this.pack(); //
	 * replace stave stave.setPhrase(phrase); stave.setKeySignature(tempKey);
	 * stave.setMetre(tempTime); stave.setBarNumbers(tempBarNumbers); }
	 */
	 ...
	 * stave.getKeySignature(); double tempTime = stave.getMetre(); boolean
	 * tempBarNumbers = stave.getBarNumbers(); // create new stave panel stave =
	 * new GrandStave(); scoreBG.removeAll(); scoreBG.add(stave);
	 * scroll.setSize(width, stave.getSize().height + 20); this.pack(); //
	 * replace stave stave.setPhrase(phrase); stave.setKeySignature(tempKey);
	 * stave.setMetre(tempTime); stave.setBarNumbers(tempBarNumbers); }
	 */
	...
	public void saveMidi() {
		...
		if (fd.getFile() != null) {
			...
			/*
			 * for(int i=0; i<staveArray.length; i++){ System.out.println(i);
			 * StavePhraseProperties props = new StavePhraseProperties(
			 * staveArray[i], staveArray[i].getPhrase()); try {
			 * System.out.println("props"); props.writeToFile(
			 * fd.getDirectory()+fd.getFile()); } catch ( Throwable e) {
	...
	private void updateAllStaves(Stave[] tempStaveArray) {
		...
		for (int i = 0; i < staveArray.length; i++) {
			...
			if (staveArray[i].getClass().isInstance(new TrebleStave()) || staveArray[i].getClass().isInstance(new BassStave())) {
				gridheightVal = 1;
