...
public class StaveActionHandler implements JMC, MouseListener, MouseMotionListener, ActionListener, KeyListener {
	...
	private MenuItem editNote, repeatNote, makeRest, deleteNote;
	private PopupMenu noteContextMenu;
	private double[] rhythmValues = { 104.0, 103.0, 102.0, 101.5, 101.0, 100.75, 100.5, 100.25, 0.0, 0.25, 0.5, 0.75, 1.0, 1.5,
			2.0, 3.0, 4.0 };
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == editNote) {
			...
			phrase.getNoteList().insertElementAt(newNote, selectedNote);
	...
	boolean inNoteArea(MouseEvent e) {
		...
		return (e.getX() <= lastX.intValue() + 15) && (e.getX() < theApp.getTotalBeatWidth() + 50);
	}
	...
	public void mouseClicked(MouseEvent e) {
		if ((((e.getModifiers() & InputEvent.BUTTON2_MASK) != 0)) && inNoteArea(e)) {
			searchForSelectedNote(e);
			if ((selectedNote >= 0) && (selectedNote < theApp.getPhrase().size())) {
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (topTimeSelected) {
			...
			if (e.getY() + theApp.staveDelta < clickedPosY - 4) {
				...
				theApp.getPhrase().setNumerator((new Double(Math.round(theApp.getMetre()))).intValue());
				clickedPosY -= 4;
				theApp.repaint();
				theApp.updateChange();
			}
	...
	public void mousePressed(MouseEvent e) {
		if (e.isPopupTrigger() || ((e.getModifiers() & InputEvent.BUTTON2_MASK) != 0) || (!theApp.editable)) {
			return;
		}
		button1Down = true;
		...
		if (!inNoteArea(e)) {
			...
			boolean white = true;
			for (int blackNote : blackNotes) {
				if (newPitch % 12 == blackNote) {
					newPitch--;
				}
			}
	...
	private void searchForSelectedNote(MouseEvent e) {
		...
		for (int i = 0; i < theApp.notePositions.size(); i += 2) {
			...
			if ((e.getX() > tempX.intValue()) && (e.getX() < tempX.intValue() + 15)
					&& (e.getY() + theApp.staveDelta > tempY.intValue() + 22)
