...
public class NoteEditor extends Dialog implements ActionListener, WindowListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8769406720713985552L;
	...
	private static boolean validateFloatEdit(TextField theField, double minValue, double maxValue) {
		...
		if (!fieldTokenizer.hasMoreElements()) {
			...
			String fieldString = fieldTokenizer.nextToken();
			try {
				double fieldValue = new Double(fieldString).doubleValue();
	...
	private TextField durationEdit = new TextField(15), dynamicEdit = new TextField(15), rhythmEdit = new TextField(15),
			panEdit = new TextField(15), offsetEdit = new TextField(15);

	private Note note;

	private Label noteLabel = new Label("Note"), dynamicLabel = new Label("Volume (1-127)"), rhythmLabel = new Label(
			"Rhythm Value"), durationLabel = new Label("Duration Factor"), panLabel = new Label("Pan"),
			offsetLabel = new Label("Offset"), octaveLabel = new Label("Octave");

	private List noteList, octaveList;

	private Button okButton = new Button("Apply"), cancelButton = new Button("Cancel");
	...
	private int getSelectedPitch() {
		...
		if (noteString.equals("Rest")) {
			...
			int octave = (new Integer(octaveList.getSelectedItem())).intValue();
			while (octave > -1) {
				answer += 12;
				--octave;
			}
	...
	private void initializeData() {
		...
		initializeDoubleEdit(durationEdit, note.getDuration() / note.getRhythmValue());

		initializeDoubleEdit(rhythmEdit, note.getRhythmValue());

		initializeDoubleEdit(offsetEdit, note.getOffset());

		initializeDoubleEdit(panEdit, note.getPan());

		initializeIntEdit(dynamicEdit, note.getDynamic());
	}
	...
	private void initializeNoteListValue(int pitch) {
		setListToMatch(noteList, getPitchStringValue(pitch));
	}

	private void initializeOctaveListValue(int pitch) {
		setListToMatch(octaveList, getOctaveStringValue(pitch));
	}

	private boolean inputIsValid() {
		return validateFloatEdit(durationEdit, 0.0000, 1.0000) && validateIntegerEdit(dynamicEdit, 0, 127)
				&& validateFloatEdit(rhythmEdit, 0.00001, 64.0000) && validateFloatEdit(panEdit, 0.0, 1.0)
				&& validateFloatEdit(offsetEdit, -999.999, 999.999);
	}
	...
	private void updateTheNote() {
		...
		note.setDuration(note.getRhythmValue() * getFieldDouble(durationEdit));
