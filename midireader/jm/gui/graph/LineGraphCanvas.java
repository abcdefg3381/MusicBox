...
public class LineGraphCanvas extends GraphCanvas {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4957409330321044935L;
	...
	public void paintBuffer() {
		...
		for (int i = 0; i < statsList.size(); i++) {
			...
			if (statsList.get(i).largestValue() != Double.POSITIVE_INFINITY && statsList.get(i).largestValue() > largestValue) {
		...
		for (int i = 0; i < statsList.size(); i++) {
			...
			for (int j = 1; j < statsList.get(i).size(); j++) {
				if (statsList.get(i).largestValue() != Double.POSITIVE_INFINITY) {
					if (i == 2) {
						graphics.drawLine((int) ((j - 1) * .5), (int) (statsList.get(i).get(j - 1) * 10000), (int) (j * .5),
								(int) (statsList.get(i).get(j) * 10000));
					} else {
						graphics.drawLine((int) ((j - 1) * .5), (int) (statsList.get(i).get(j - 1) * 300), (int) (j * .5),
								(int) (statsList.get(i).get(j) * 300));
					}
				}
			}
		}
