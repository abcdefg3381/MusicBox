...
public class WaveView implements ActionListener, ComponentListener {
	...
	// menu items
	private MenuItem size1, size2, size4, size8, size16, size32, size64, size128, size256, size512, size1024, size2048,
			openFile, quit, changeColor, vSmall, small, medium, large, huge, times1, times2, times3, times4;
	...
	private void drawWave(int xLoc, int yLoc) {
		...
		/*
		 * size4096 = new MenuItem("1:4096"); size4096.addActionListener(this);
	...
	public void openFile() {
		FileDialog loadFile = new FileDialog(f, "Select a 16 bit audio file in .au format (no compression).", FileDialog.LOAD);
	...
	/**
	 * Pause playback of an audio file jMusic audio playback via javaSound. This
	 * method requires the javax.sound packages in Java 1.3 or higher.
	 */
	...
	public void playFile() {
		if (channels > 2) {
			...
			System.out.println("---- Playing audio file '" + getFileName() + "'... Sample rate = " + afr.getSampleRate()
					+ " Channels = " + afr.getChannels() + " ----");
	...
	public void setStartPos(int newOffset) {
		...
		// f.repaint();
		/*
		 * for (int i=0; i<channels; i++) { canvases[i].setFastDraw(true);
		 * canvases[i].repaint(); }
		 */
		for (int i = 0; i < channels; i++) {
	...
	/**
	 * Continue playback an audio file jMusic audio playback via javaSound. This
	 * method requires the javax.sound packages in Java 1.3 or higher.
	 * 
	 * @param String
