...
/**
 * This class displays floating point data passed to it, typically read from the
 * audio file or generated by s synthesis process. It is mainly used in
 * conjunction with the WaveView class.
 * 
 * @author Andrew Brown
 */
...
public class WaveCanvas extends Canvas {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3940882154749233271L;
	...
	public void paint(Graphics graphics) {
		...
		// int pixCount = Math.min(data.length-resolution, this.getSize().width
		// * resolution);
		...
		if (resolution == 1) {
			for (int i = 0; i < xMax && i < dataMax; i++) {
				currData = data[i];
				g.drawLine(i, (int) (h2 - currData * h2 * amplitude), i, (int) (h2 - currData * h2 * amplitude));
			}
		} else {
			int xPosition = 0;
			float max = 0.0f;
			...
			for (int i = 0; i < xMax && i < dataMax; i += resolution) {
				...
				if (fastDraw) {
					g.drawLine(xPosition++, (int) (h2 - currData * h2 * amplitude), xPosition, (int) (h2 - currData * h2
							* amplitude));
				} else {
					// max and min
					max = 0.0f;
					...
					if (resolution > 8) {
						...
						if (max > 0.0f) {
							g.drawLine(xPosition, (int) (h2 - drawMax * h2 * amplitude), xPosition, (int) (h2 - max * h2
									* amplitude));
						}
						if (min < 0.0f) {
							g.drawLine(xPosition, (int) (h2 - drawMin * h2 * amplitude), xPosition, (int) (h2 - min * h2
									* amplitude));
						}
					}
					// draw wave
					g.drawLine(xPosition++, (int) (h2 - currData * h2 * amplitude), xPosition, (int) (h2 - data[i + resolution]
							* h2 * amplitude));
				}
			}
		}
	 ...
	 * 
	 * @param state
	 *            - True = quick rendering, false = more accurate rendering
	 */
