...
/**
 * The Phrase class is representative of a single musical phrase. Phrases are
 * held in Parts and can be played at any time based on there start times. They
 * may be played sequentially or in parallel. Phrases can be added to an Part
 * like this...
 * 
 * <pre>
...
public class Phrase implements JMC, Cloneable, Serializable {
	private final class Position implements Serializable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 264325204651751138L;
		...
		private Position(final Position anchor, final Alignment alignment, final double offset,
				final Phrase phrase) {
		...
		private final Position copy(final Phrase newCopy) {
			return (isAbsolute) ? new Position(startTime, newCopy) : new Position(anchor,
					alignment, offset, newCopy);
		}
		...
		private final double getStartTime() {
			if (isAbsolute) {
				...
				return alignment.determineStartTime(phrase.getTotalDuration(),
						anchor.getStartTime(), anchor.getEndTime())
						+ offset;
			}
		}
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1523085852474908130L;
	/**
	 * Setting the phrase to append when added to a part rather than use its
	 * start time.
	 */
	...
	/**
	 * The pan position for notes in this phrase. This must be set delibertley
	 * to override a note's pan position.
	 */
	...
	// ----------------------------------------------
	/**
	 * Creates an empty Phrase. The default start time is a flag which means the
	 * phrase will be appended to the end of any part it is added to.
	 */
	public Phrase() {
	 ...
	 * 
	 * @param startTime
	 *            The beat at which the phrase will be positioned in its part.
	 */
	 ...
	 *            The beat at which the phrase will be positioned in its part.
	 * @param instrument
	 *            The sound or instrument number to be used for this phrase.
	 */
	 ...
	 * 
	 * @param note
	 *            Note to be containing by the phrase.
	 */
	 ...
	 *            Note to be containing by the phrase.
	 * @param title
	 *            String describing the title of the Phrase.
	 */
	 ...
	 * 
	 * @param notes
	 *            array of Note to be contained by the phrase.
	 */
	 ...
	 *            The sound or instrument number to be used for this phrase.
	 * @param append
	 *            A flag specifying wheather or not this phrase should be added
	 *            to the end of the part it is added too, or should use its
	 *            start time value.
	 */
	 ...
	 * 
	 * @param Note
	 *            note - add a note to this phrase
	 */
	...
	/**
	 * Adds Multiple notes to the phrase all of which start at the same time and
	...
	/**
	 * Adds Multiple notes to the phrase from one pitch and an array of rhythm
	 * values
	 * 
	 * @param frequency
	 *            The pitch values for the notes in hertz
	 * @param rhythms
	 *            An array of rhythm values
	 */
	public void addNoteList(double frequency, double[] rhythms) {
		for (double rhythm2 : rhythms) {
			this.addNote(new Note(frequency, rhythm2));
		}
	}
	 ...
	 * 
	 * @param pitchAndRhythmArray
	 *            - an array of pitch and rhythm values
	 */
	public void addNoteList(double[] pitchAndRhythmArray) {
		for (int i = 0; i < pitchAndRhythmArray.length; i += 2) {
			try {
				...
				System.err
						.println("Error adding note list: Possibly the wrong number of values in the pitch and rhythm array.");
			}
		}
	}
	...
	/**
	 * Adds Multiple notes to the phrase from several arrays A boolean option
	 * when true appends the notes to the end of the list if non true the
	 * current list is errased and replaced by the new notes
	 * 
	 * @param freqArray
	 *            array of frequency values
	 * @param rhythmArray
	 ...
	 * 
	 * @param pitch
	 *            The pitch values for the notes
	 * @param rhythms
	 *            An array of rhythm values
	 */
	public void addNoteList(int pitch, double[] rhythms) {
		for (double rhythm2 : rhythms) {
			this.addNote(new Note(pitch, rhythm2));
		}
	}
	...
	/**
	 * Adds Multiple notes to the phrase from a pitch array, rhythm value, and
	 * dynmaic value
	 * 
	 * @param pitchArray
	 *            - An array of pitch values
	...
	/**
	 * Adds an array of notes to the phrase. A boolean option when true appends
	 * the notes to the end of the list.
	 * 
	 * @param noteArray
	 *            the array of notes to add
	 * @param append
	 *            do we append or not?
	 */
	...
	/**
	 * Adds a vector of notes to the phrase. A boolean option when true appends
	 * the notes to the end of the list, if false the notes in noteVector will
	 * replace the notes currently in the phrase.
	 * 
	 * @param noteVector
	 *            the vector of notes to add
	 * @param append
	 *            do we append or not?
	 */
	public void addNoteList(Vector noteVector, boolean append) {
		...
		while (enum1.hasMoreElements()) {
			try {
				...
				System.err
						.println("The vector passed to this method must " + "contain Notes only!");
			}
		}
	}
	 ...
	 * 
	 * @param Rest
	 *            rest - The rest to be added to this phrase
	 */
	...
	/**
	 * Returns a carbon copy of a specified Phrase Changes to notes in the
	 * original or the alias will be echoed in the other. Note: that for this to
	 * work other phrase classes must to change the noteList attribute to point
	 * to another object, but instead should always update the noteList itself.
	 * See shuffle() as an example.
	 */
	public Phrase alias() {
	...
	/**
	 * <p>
	 * The positions tries the phrase relative to another using the alignment
	 * specified. If the arrangement causes this class to start before a start
	 * time of 0.0, the repositioning is considered invalid and will fail. The
	 * original positioning will be restored and this method will return false.
	 * 
	 * If successful, the previous positioning whether absolute or relative will
	 * be lost.
	 * 
	 * <p>
	 * To position this absolutely use the <code>setStartTime</code> method
	 * instead.
	 * 
	 * @param anchor
	 *            the phrase against which this should be positioned
	 * @param alignment
	 *            how this should be positioned relative to anchor
	 * @returns false if anchoring failed due to being positioned before the 0.0
	 *          start time barrier. True otherwise.
	 */
	public boolean attemptAnchoringTo(final Phrase anchor, final Alignment alignment,
			final double offset) {
	...
	/**
	 * Returns a copy of a specified section of the Phrase, pads beginning and
	 * end with shortedend notes and rests if notes or phrase boundaries don't
	 * align with locations.
	 * 
	 ...
	 * @param double start location
	 * @param double end location
	 * @param boolean requireNoteStart if true only notes that start inside the
	 *        copy range are included in the copy. Notes starting prior but
		...
		for (int i = 0; i < this.size(); i++) {
			...
			if (beatCounter < startLoc) { // this note starts before the space
				if ((beatCounter + this.getNote(i).getRhythmValue() > startLoc)
						&& (beatCounter + this.getNote(i).getRhythmValue() <= endLoc)) { // ends
																							// within
																							// the
																							// space
					if (requireNoteStart) {
						Note n = new Note(REST, beatCounter + this.getNote(i).getRhythmValue()
								- startLoc, this.getNote(i).getDynamic());
						tempPhr.addNote(n);
					} else {
						if (this.getNote(i).getPitchType() == Note.MIDI_PITCH) {
							Note n = new Note(this.getNote(i).getPitch(), beatCounter
									+ this.getNote(i).getRhythmValue() - startLoc, this.getNote(i)
									.getDynamic());
							tempPhr.addNote(n);
						} else {
				...
				if (beatCounter + this.getNote(i).getRhythmValue() > endLoc) { // ends
																				// after
																				// the
																				// space
					if (requireNoteStart) {
						...
						if (this.getNote(i).getPitchType() == Note.MIDI_PITCH) {
							Note n = new Note(this.getNote(i).getPitch(), beatCounter + endLoc
									- startLoc, this.getNote(i).getDynamic());
							tempPhr.addNote(n);
						} else {
							Note n = new Note(this.getNote(i).getPitch(), beatCounter + endLoc
			...
			if (beatCounter >= startLoc && beatCounter < endLoc) { // this note
																	// starts in
																	// the space
				if (beatCounter + this.getNote(i).getRhythmValue() <= endLoc) { // also
																				// ends
																				// in
																				// it
					tempPhr.addNote(this.getNote(i));
				} else { // ends after the end. Make up last note.
					Note n = new Note(this.getNote(i).getPitch(), endLoc - beatCounter, this
							.getNote(i).getDynamic());
					tempPhr.addNote(n);
				}
			}
	 ...
	 * align with locations.
	 * 
	 * @param boolean trimmed wether to truncte notes (as per the other versions
	 *        of copy) or not
	 * @param boolean startTimeShifts wether to shift the start time or to add a
	 *        rest if if the start is afte startloc
	 * @param double start location
	 * @param double end location
	 * @return Phrase a copy of the Phrase
	 */
	public Phrase copy(double startLoc, double endLoc, boolean trimmed, boolean truncated,
		...
		// make beatCounter add up to the right amount before going though the
		// segment
		...
		// now it is in the segment, should a rest be added in the begining
		// because
		// a note overlaps?
		if (startLoc < beatCounter) {
	...
	/**
	 * Returns a copy of the entire Phrase only ontaining notes between highest
	 * and lowset specified pitch. @ param highestPitch The top MIDI pitch to
	 * include in the copy @ param lowestPitch The bottom MIDI pitch to include
	 * in the copy
	 * 
	...
	/**
	 * Generates and returns a new note with default values and adds it to this
	 * phrase.
	 */
	 ...
	 * 
	 * @returns null if aligned with setStartTime(), or details of alignment if
	 *          aligned with attemptAnchoringTo()
	 */
	 ...
	 * 
	 * <p>
	 * To position this relative to another class use the <code>anchor</code>
	 * method instead.
	 * 
	...
	public void setStartTime(double startTime) {
		if (startTime >= MIN_START_TIME) {
			...
			System.err
					.println("Error setting phrase start time value: You must enter values greater than "
							+ MIN_START_TIME);
		}
	}
	...
	public String toString() {
		String phraseData = new String("-------- jMusic PHRASE: '" + title + "' contains "
				+ this.size() + " notes.  Start time: " + getStartTime() + " --------" + '\n');
		if (this.tempo > 0) {
