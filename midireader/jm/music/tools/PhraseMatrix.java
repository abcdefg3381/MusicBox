...
/**
 * The PhraseMatrix class holds AdaptiveArrays for each Note parameter type and
 * is also responsible for any mappings that need to occur between Note types
 * and the AdaptiveMatrix
 * 
...
public final class PhraseMatrix implements JMC {
	...
	/**
	 * The array of notes associated with the original phrase. This information
	 * is kept so that we can generate new notes while still keeping any of the
	 * phrases original note attributes.
	 */
	private Note[] notes;
	...
	/**
	 * A map from rhythm values to AdaptiveMatrix index values.
	 */
	...
	// --------------------------------------------------
	/**
	 * Takes a phrase as input and creates matrix's for pitch, rhythm and
	 * dynamic from the phrase's notes at the specifed depth.
	 */
	...
	/**
	 * Takes a phrase as input and creates matrix's for pitch, rhythm and
	 * dynamic from the phrase's notes at different depths of each attribute.
	 */
	...
	/**
	 * Generates a the number of notes requrested using a combination of
	 * generated note attributes and existing note attributes. This method works
	 * on the existing length of note data available from the existing phrase.
	 * 
	 * @param p
	 *            boolean consider the pitches or not
	 * @param r
	 *            boolean consider the rhythmValues or not
	 * @param d
	 *            boolean consider the dynamics or not
	...
	/**
	 * Generates a the number of notes requrested using a combination of
	 * generated note attributes and existing note attributes.
	 * 
	 * @param p
	 *            boolean consider the pitches or not
	 * @param r
	...
	public Phrase generate(boolean p, boolean r, boolean d, int numOfNotes) {
		...
		/*
		 * for(int i=0;i<this.depth;i++){ pitch[i] = notes[i].getPitch();
		 * dynamic[i] = notes[i].getDynamic(); for(int
		 * j=0;j<rhythmMap.length;j++){ if(notes[i].getRhythmValue() ==
		 * rhythmMap[j]){ rhythm[i] = j; break; } } }
		 */
		// pitch
