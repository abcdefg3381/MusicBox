...
public final class ChordAnalysis {
	...
	private static boolean acceptableChange(final int[] chords, final int dominantIndex, final int previous) {
	...
	private static Possible findCommonChords(final int[] firstChords, final int[] secondChords) {
		Possible returnChords = new Possible(new int[2]);
		int index = 0;
		for (int firstChord : firstChords) {
			for (int secondChord : secondChords) {
	...
	private static Possible firstPass(final Note downBeat, final Note halfBeat, final int tonic, final int[] scale,
			final int[][] triads) {
		if (isBad(downBeat, tonic, scale)) {
			...
			if (isBad(halfBeat, tonic, scale)) {
				...
				if (PhraseAnalysis.pitchToDegree(downBeat.getPitch(), tonic) == PhraseAnalysis.pitchToDegree(
						halfBeat.getPitch(), tonic)) {
					return firstPassChords(downBeat, tonic, scale, triads);
				} else {
					return firstPassChords(downBeat, halfBeat, tonic, scale, triads);
				}
			}
		}
	}

	private static Possible firstPassChords(final Note note, final int tonic, final int[] scale, final int[][] triads) {
	...
	private static Possible firstPassChords(final Note note1, final Note note2, final int tonic, final int[] scale,
			final int[][] triads) {
		Possible firstChords = firstPassChords(note1, tonic, scale, triads);
		Possible secondChords = firstPassChords(note2, tonic, scale, triads);
		Possible commonChords = findCommonChords(firstChords.chords, secondChords.chords);
		return (commonChords == null) ? firstChords : commonChords;
	}

	/**
     */
	public static Possible[] getChords(final Phrase phrase, final double beatLength, final int tonic, final int[] scale) {
		...
		final Possible[] chords = new Possible[(int) Math.ceil(endTime / beatLength)];
	...
	public static int[] getFirstPassChords(final Phrase phrase, final double beatLength, final int tonic, final int[] scale) {
	...
	public static int[] getSecondPassChords(final Phrase phrase, final double beatLength, final int tonic, final int[] scale) {
		Possible[] chords = getChords(phrase, beatLength, tonic, scale);
		int[] returnChords = new int[chords.length];
		...
		outerLoop: while (index > 0) {
			...
			if (acceptableChange(chords[index].chords, dominantIndex, chords[index].getBestChord())) {
