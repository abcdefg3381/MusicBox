...
public class ClimaticPopInitialiser extends PopulationInitialiser {
	...
	public ClimaticPopInitialiser(int population) {
		...
		panel.add(new Scrollbar(Scrollbar.HORIZONTAL, populationSize, 1, MIN_POPULATION_SIZE, MAX_POPULATION_SIZE) {
			/**
			 * 
			 */
			private static final long serialVersionUID = 7798322423578888907L;
	...
	private void addNote(Phrase phrase, Note target, int targetBeat, double rhythmValue, int[] intervalArray, int climax,
		...
		if (rhythmValue < 0.0) {
			...
			double originalRatio = (target.getPitch() - previousPitch) / (targetBeat - phrase.getEndTime());
			int selectedInterval = intervalArray[(int) (Math.random() * intervalArray.length)];
			...
			double currentRatio = currentInterval / (targetBeat - phrase.getEndTime());
	...
	private void extend(Phrase phrase, Note target, int targetBeat, double[][] beatArray, int[] intervalArray, int climax,
		...
		while (length < targetBeat) {
			if (length == 2 * beatsPerBar) {
				...
				if (!(previousPitch % 12 == TONIC % 12 || previousPitch % 12 == (TONIC + 7) % 12)) {
					int nextHarmoniousNote = previousPitch + 1;
					while (!(nextHarmoniousNote % 12 == TONIC % 12 || nextHarmoniousNote % 12 == (TONIC + 7) % 12)) {
						nextHarmoniousNote++;
					}
					int prevHarmoniousNote = previousPitch - 1;
					while (!(prevHarmoniousNote % 12 == TONIC % 12 || prevHarmoniousNote % 12 == (TONIC + 7) % 12)) {
						prevHarmoniousNote--;
					}
					...
					} else if (nextHarmoniousNote - previousPitch > previousPitch - prevHarmoniousNote) {
						previousPitch = prevHarmoniousNote;
					} else if (previousPitch - prevHarmoniousNote > nextHarmoniousNote - previousPitch) {
						previousPitch = nextHarmoniousNote;
					} else {
						previousPitch = nextHarmoniousNote;
					}
				...
				// addNote(phrase, target, targetBeat, 1.0, intervalArray,
				// climax,
				// lowerlimit);
				// }
			} else {
				...
				// Don't add group of beats that will not cross the next bar
				// line
				while (counter < 30 && length + beatsInArray > (length / beatsPerBar + 1) * beatsPerBar) {
					beatIndex = (int) (Math.random() * beatArray.length);
					double tempRVCount = 0;
					for (int i = 0; i < beatArray[beatIndex].length; i++) {

						tempRVCount += (beatArray[beatIndex][i] < 0) ? 0 - beatArray[beatIndex][i] : beatArray[beatIndex][i];
					}
				...
				if (counter != 30) {
					// Add beats to melody
					for (int i = 0; i < beatArray[beatIndex].length; i++) {
						addNote(phrase, target, targetBeat, beatArray[beatIndex][i], intervalArray, climax, lowerlimit);
					}
				} else {
					addNote(phrase, target, targetBeat, ((length / beatsPerBar + 1) * beatsPerBar) - length, intervalArray,
							climax, lowerlimit);
				}
			}
	...
	private double[][] generateBeatRhythmArray(Phrase phrase, int beatsPerBar) {
		...
		while (absoluteNotesProcessed < phrase.size()) {
			...
			while (cumulativeRV < originalCumulativeRV + beatsPerBar && notesProcessed < phrase.size()) {
				note = phrase.getNote(notesProcessed++);
				rhythmValue = note.getRhythmValue();
				tempRVArray[count++] = rhythmValue;
				if (note.getPitch() == Note.REST) {
				...
				if (cumulativeRV <= originalCumulativeRV + beatsPerBar) {
	...
	private Note generateClimax(int lowestPitch) {
		...
		while (pitch == 0) {
			if (currentPitch % 12 == TONIC % 12 || currentPitch % 12 == (TONIC + 7) % 12) {
				pitch = currentPitch;
			}
			currentPitch++;
		}
	...
	public Phrase[] initPopulation(final Phrase phrase, final int beatsPerBar) {
		...
		for (int i = 0; i < populationSize; i++) {
			...
			// Extend to target
			extend(population[i], target, targetBeat, beatRhythmArray, intervalArray, climax, beatsPerBar, lowerlimit);
	...
	private boolean isClimaxAccepted(Phrase phrase, int beatsPerBar) {
		...
		if (lastHighestPitch > phrase.getNote(0).getPitch() + 12 && repetitions <= 1) {
			return true;
		}
		return false;
	}
	...
	private boolean isScale(int pitch) {
		for (int element : PhraseAnalysis.MAJOR_SCALE) {
			if (pitch % 12 == element) {
				return true;
			}
		}
