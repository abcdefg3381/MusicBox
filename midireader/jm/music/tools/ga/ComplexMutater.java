...
public class ComplexMutater extends Mutater {
	...
	private int[] MUTATE_PERCENTAGE = { 0, 40, 1, 40, 60 }; // {1, 50, 5, 10,
															// 40}; // {1, 50,
															// 0, 0, 40};
	...
	public ComplexMutater() {
		...
		scrollbar = new Scrollbar(Scrollbar.HORIZONTAL, MUTATE_PERCENTAGE[0], 1, 0, 100);
		choice = new Choice();
		...
		choice.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent evt) {
				mutateLabel.setText(Integer.toString(MUTATE_PERCENTAGE[choice.getSelectedIndex()]));
		...
		scrollbar.addAdjustmentListener(new AdjustmentListener() {
			...
			public void adjustmentValueChanged(AdjustmentEvent evt) {
				MUTATE_PERCENTAGE[choice.getSelectedIndex()] = scrollbar.getValue();
	...
	private Vector applyTonalPausesMutation(final Phrase phrase, double initialLength, int initialSize, int beatsPerBar) {
		...
		for (int j = initialSize; j < phrase.size() - 1; j++) {
			...
			if (rhythmValueCount / beatsPerBar == Math.ceil(rhythmValueCount / beatsPerBar) && (degree == 0 || degree == 7)
					&& Math.random() < (2.0 / rhythmValue) * (MUTATE_PERCENTAGE[4] / 100.0)) {
	...
	/**
	 * This class incoproates a visual editor to change the amount of each
	 * mutation, the returned awt panel can be put into a visible componenet to
	 * allow GUI editing of the mutation amounts.
	 */
	@Override
	...
	public Phrase[] mutate(Phrase[] population, double initialLength, int initialSize, int beatsPerBar) {
		...
		for (Phrase individual : population) {
			...
			if (Math.random() < MUTATE_PERCENTAGE[1] / 100.0) {
				...
				beatCount = 0;
				for (int j = 0; j < individual.size(); j++) {
					if (beatCount / beatsPerBar == Math.floor(beatCount / beatsPerBar)) {
						notesOnBars[index] = j;
			...
			for (int j = 0; j < n2change3; j++) {
				...
				if (rhythmValue5 >= 1.0 && rhythmValue5 % 1.0 == 0 && rhythmValue5 * 2.0 == Math.ceil(rhythmValue5 * 2.0)) {
			...
			while (k < vector.size()) {
				...
				if (currentPitch != Note.REST) {
					...
					if ((Math.abs(interval) == 4 || Math.abs(interval) == 3) && Math.random() < (MUTATE_PERCENTAGE[3] / 100.0)) {
						int scalePitch = 0;
						if (interval > 0) {
						...
						if (currentRV > previousRV) {
							if (currentRV >= 0.5 && (int) Math.ceil(currentRV * 2) == (int) (currentRV * 2)) {
							...
							if (previousRV >= 0.5 && (int) Math.ceil(previousRV * 2) == (int) (previousRV * 2)) {
			...
			individual.addNoteList(applyTonalPausesMutation(individual, initialLength, initialSize, beatsPerBar), false);
