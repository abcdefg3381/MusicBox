 ...
 * @author Andrew Brown
 * @author Adam Kirby
 * @version $Revision: 1.6 $, $Date: 2003/10/23 03:28:04 $
 */
public class Mod implements JMC {

	/**
	 * Increase the dynamic of notes at regularly occuring pulse locations which
	 * generates the sound of regular meter by emphasising the first beat of
	 * each bar/measure.
	 * 
	 * <P>
	 ...
	 * @param meter
	 *            double describing the number of croctets per bar/measure
	 */
	...
	/**
	 * Increase the dynamic of notes at regularly occuring pulse locations which
	 * generates the sound of regular meter by accenting specified beats within
	 * each bar/measure.
	 * 
	 * <P>
	 ...
	 *            double describing the number of croctets per bar/measure
	 * @param accentedBeats
	 *            double array describing the time of the accents in the bar.
	 */
	public static void accents(CPhrase cphrase, final double meter, final double[] accentedBeats) {
		accents(cphrase, meter, accentedBeats, 20);
	}

	/**
	 * Increase the dynamic of notes at regularly occuring pulse locations which
	 * generates the sound of regular meter. The meter is the total beat length
	 * of the bar/measure, while the array of accented beats is the times within
	 * the bar at which an accent should be made. For example, 6/8 time would be
	 * implied by a meter of 3 and a accent array of {0.0, 1.5}.
	 * 
	 * <P>
	 * <CODE>accentedBeats</CODE> is an array of values describing where the
	 * accents begin relative to the start of each bar, in crochets.
	 ...
	 *            double array describing the time of the accents in the bar.
	 * @param accentAmount
	 *            integer describing the value that the dynamic of accented
	 *            beats are increased by.
	 */
	public static void accents(CPhrase cphrase, final double meter, final double[] accentedBeats, final int accentAmount) {
		if (cphrase == null || meter <= 0.0) {
			return;
		}
		for (double accentedBeat : accentedBeats) {
			if (accentedBeat < 0.0 || accentedBeat >= meter) {
				return;
			}
		}
	 ...
	 *            beats are increased by.
	 */
	public static void accents(Part part, final double meter, final double[] accentedBeats, final int accentAmount) {
	 ...
	 *            beats are increased by.
	 */
	public static void accents(Phrase phrase, final double meter, final double[] accentedBeats, final int accentAmount) {
		...
		for (int i = 0; i < v.size(); i++) {
			...
			for (double accentedBeat : accentedBeats) {
				if (beatCounter % meter == accentedBeat) {
	 ...
	 * @param meter
	 *            double describing the number of crochets per bar/measure
	 */
	 ...
	 *            double array describing the time of the accents in the bar.
	 */
	public static void accents(Score score, final double meter, final double[] accentedBeats) {
	 ...
	 *            beats are increased by.
	 */
	public static void accents(Score score, final double meter, final double[] accentedBeats, final int accentAmount) {
	 ...
	 * @param amount
	 *            - The number of beats to add to the duration.
	 */
	 ...
	 * @param note2
	 *            Note that passes on its rhythm features
	 */
	 ...
	 *            the Part to be appended
	 * @param fromLoc
	 *            double describing the start time for the second part to be
	 *            appended
	 */
	 ...
	 * from note to note. This process only effects the pan value of the notes
	 * in the cphrase.
	 * <P>
	 ...
	 * @param newLength
	 *            double describing the number of beats to change the phrase to.
	 */
	 ...
	 * @param retio
	 *            double describing the compression factor.
	 */
	public static void compress(CPhrase cphrase, final double ratio) {
		...
		Enumeration enum1 = cphrase.getPhraseList().elements();
		while (enum1.hasMoreElements()) {
			compress((Phrase) enum1.nextElement(), ratio);
		}
	}
	...
	/**
	 * A compressor/expander routine. Compression ratio numbers between 0 and 1
	 * compress, values larger than 1 expand. Negative values invert the dynamic
	 * about the mean.
	 * 
	 * <P>
	 ...
	 * {@link Note#MIN_DYNAMIC MIN_DYNAMIC} then the dynamic will be set to a
	 * value as described in the {@link Note#setDynamic} method.
	 * 
	 * <P>
	...
	/**
	 * Pack all phrases from parts with the same channel into one part. Data
	 * from all parts with the same channel is consolidated into one part. This
	 * is often useful after appending several scores with simmilar
	 * instrumentation.
	 * 
	 * <P>
	 * If <CODE>score</CODE> is null then this method does nothing.
	...
	public static final void crescendo(final Phrase phrase, final double startTime, final double endTime,
	...
	public static void cycle(Part part, double to) {
		...
		for (cycleAt = to; (int) (cycleAt / endTime) > 1; cycleAt -= endTime) {
			...
			for (Phrase phrase : phrases) {
				...
				phrase.setStartTime(phrase.getStartTime() + startPoint * endTime);
	 ...
	 *            Phrase to be cycled
	 * @param numBeats
	 *            double describing the minimum length of the final phrase, in
	 *            crotchets
	 */
	 ...
	 *            Phrase to be cycled
	 * @param numbNotes
	 *            the number of notes in the final phrase
	 */
	...
	public static final void decrescendo(final Phrase phrase, final double startTime, final double endTime,
			final int startDynamic, final int endDynamic) {
		crescendo(phrase, startTime, endTime, startDynamic, endDynamic);
	}

	public static final void diminuendo(final Phrase phrase, final double startTime, final double endTime,
			final int startDynamic, final int endDynamic) {
		crescendo(phrase, startTime, endTime, startDynamic, endDynamic);
	}
	 ...
	 *            - The CPhrase to be modified.
	 * @param amount
	 *            - The scaling multiplyer for the intervals, i.e., 2.0 doubles
	 *            width.
	 */
	 ...
	 *            CPhrase to be faded
	 * @param fadeLength
	 *            double describing the time of the fade, in crotchets.
	 */
	 ...
	 *            double describing how far into the fade the phrase starts.
	 */
	public static void fadeIn(CPhrase cphrase, final double fadeLength, final double cphraseStartTime) {
		...
		while (enum1.hasMoreElements()) {
			...
			fadeIn(nextPhrase, fadeLength, (cphraseStartTime + nextPhrase.getStartTime()));
		}
	}
	 ...
	 *            Part to be faded
	 * @param fadelength
	 *            double describing the number of crotchets to fade over
	 */
	 ...
	 *            Part to be faded
	 * @param fadeLength
	 *            double describing the duration of the fade, in crotchets
	 * @param partStartTime
	 *            double describing how far into the fade the part starts
	 */
	public static void fadeIn(Part part, final double fadeLength, final double partStartTime) {
	 ...
	 *            Phrase to be faded
	 * @param fadelength
	 *            double describing the number of beats (crotchets) to fade over
	 */
	public static void fadeIn(Phrase phrase, final double fadeLength) {
		...
		while (enum1.hasMoreElements()) {
			...
			int dynamic = (int) (nextNote.getDynamic() * fadeFactor);
			if (dynamic == 0) {
	 ...
	 *            double describing how far into the fade the phrase starts
	 */
	public static void fadeIn(Phrase phrase, final double fadeLength, final double phraseStartTime) {
	 ...
	 *            CPhrase to be faded
	 * @param fadeLength
	 *            double describing the time of the fade out in crotchets
	 */
	public static void fadeOut(CPhrase cphrase, final double fadeLength) {
		...
		while (enum1.hasMoreElements()) {
			...
			// as the CPhrase does
			fadeOut(nextPhrase, fadeLength, (cphrase.getEndTime() - nextPhrase.getEndTime()));
		}
	}
	 ...
	 *            double describing the time of the fade out in crotchets
	 * @param phraseEndTime
	 *            double describing the length of time, in crothcets, between
	 *            the end of the phrase and the end of the fade.
	 */
	public static void fadeOut(CPhrase cphrase, final double fadeLength, final double cphraseEndTime) {
		...
		while (enum1.hasMoreElements()) {
			...
			// make the correction for phrases that don't end at the same time
			// as the Part does
			fadeOut(nextPhrase, fadeLength, (cphraseEndTime + cphrase.getEndTime() - nextPhrase.getEndTime()));
		}
	}
	 ...
	 *            Part to be faded
	 * @param fadeLength
	 *            double describing the duration of the fade out in crotchets
	 */
	 ...
	 *            double describing the duration of the fade out in crotchets
	 * @param partEndTime
	 *            double describing the length of time, in crotchets, between
	 *            the end of the part and the end of the fade.
	 */
	public static void fadeOut(Part part, final double fadeLength, final double partEndTime) {
	...
	public static void fadeOut(Phrase phrase, final double fadeLength) {
		...
		double rhythmValueCounter = 0.0;
		int phraseLength = phrase.size() - 1;// -1 due to Vector elements
												// starting at 0
		for (int i = 0; i <= phraseLength; i++) {
			...
			double fadeFactor = rhythmValueCounter / fadeLength;
			int dynamic = (int) (nextNote.getDynamic() * fadeFactor);
			if (dynamic == 0) { // only fade out to dynamic of 1 as 0 is note
								// off
				dynamic = 1;
			}
			nextNote.setDynamic(dynamic);
	 ...
	 *            the end of the phrase and the end of the fade.
	 */
	public static void fadeOut(Phrase phrase, final double fadeLength, final double phraseEndTime) {
	...
	public static void fadeOut(Score score, final double fadeLength) {
		...
		while (enum1.hasMoreElements()) {
			Part nextPart = (Part) enum1.nextElement();
			// make the correction for Parts that don't end at the same time as
			// the Score does
	 ...
	 * creating one longer note and deleting the rest. This modification may
	 * reduce the overall note count in the phrase, be careful that use of this
	 * method does not to cause array out of bounds exceptions.
	 * 
	 * @param CPhrase
	 ...
	 * one longer note and deleting the rest. This modification may reduce the
	 * overall note count in the phrase, be careful that use of this method does
	 * not to cause array out of bounds exceptions.
	 * 
	 * @param Part
	 ...
	 * mirrored. That is, notes which are n semitones above the first pitch will
	 * be changed to be n semitones below.
	 * 
	 * <P>
	 ...
	 *            the base CPhrase
	 * @param cphrase2
	 *            the CPhrase to be merged with the first
	 */
	...
	/**
	 * Combines the parts from a second score into the first one. Parts on an
	 * existing channel will have phases extracted and added to that part,
	 * otherwise a part with a unique channel number will be added.
	 * <P>
	 * If <CODE>score1</CODE> or <CODE>score2</CODE> is null then this method
	 * does nothing.
	 * 
	 ...
	 *            the base score
	 * @param score2
	 *            the Score to be merged to the base score
	 */
	...
	public static void mutate(Phrase phrase) {
		mutate(phrase, 1, 1, CHROMATIC_SCALE, phrase.getLowestPitch(), phrase.getHighestPitch(), new double[] { 0.25, 0.5, 1.0,
				1.5, 2.0 });
	}

	/**
	 * Mutates the <CODE>phrase</CODE> by changing pitches and rhythm values.
	 * The number of pitches and rhythm values to change can be set, and the
	 * pitches and rhythmValues to select from can be specified. The notes
	 * within the phrase to change are selected at random. The scale values need
	 * to be within 0-11 and will typically use the JMC scale constants.
	 * 
	 * @param Phrase
	 *            Phrase to be mutated
	 ...
	 *        replacements
	 */
	public static void mutate(Phrase phrase, int pitchCount, int rhythmCount, int[] pitches, int lowestPitch, int highestPitch,
			double[] rhythms) {
		// pitch mutation
	...
	public static void mutate(Phrase phrase, int pitchCount, int[] scale) {
		mutate(phrase, 1, 0, scale, phrase.getLowestPitch(), phrase.getHighestPitch(), new double[] {});
	}
	...
	public static void normalise(CPhrase cphrase) {
		...
		// get the curent max
		int max = 0;
		Enumeration enum0 = cphrase.getPhraseList().elements();
		while (enum0.hasMoreElements()) {
			Phrase phrase = (Phrase) enum0.nextElement();
			Enumeration enum1 = phrase.getNoteList().elements();
			while (enum1.hasMoreElements()) {
				Note n = (Note) enum1.nextElement();
				if (n.getDynamic() > max) {
	...
	public static void normalise(Part part) {
		...
		// get the curent max
		int max = 0;
		Enumeration enum0 = part.getPhraseList().elements();
		while (enum0.hasMoreElements()) {
			Phrase phrase = (Phrase) enum0.nextElement();
			Enumeration enum1 = phrase.getNoteList().elements();
			while (enum1.hasMoreElements()) {
	 ...
	 *            boolean specifying whether the last note is to be repeated
	 */
	 ...
	 * @param qValue
	 *            the beat subdivision value to quantise to, in crotchets
	 */
	 ...
	 *            Score to quantise
	 * @param qValue
	 *            double describing the amount to quantise to
	 */
	 ...
	 * @param qValue
	 *            the rhythm value to quantize to, in crotchets
	 */
	 ...
	 *            Score to quantize
	 * @param qValue
	 *            double describing the amount to quantize to
	 */
	...
	/**
	 * Adjusts Note values to any value plus or minus a specified amount. This
	 * process can effect pitch, rhythm & duration, dynamic, and pan values of
	 * the notes in the cphrase.
	 * <P>
	 * If <CODE>cphrase</CODE> is null then this method does nothing.
	 * 
	 * @param cphrase
	 ...
	 *            The degree of rhythm value change to apply.
	 */
	...
	public static void randomize(Phrase phrase, int pitchVariation, double rhythmVariation, int dynamicVariation) {
		...
		while (enum1.hasMoreElements()) {
			...
			if (pitchVariation > 0) {
				n.setPitch(n.getPitch() + (int) (Math.random() * (pitchVariation * 2) - pitchVariation));
			}
			...
			if (dynamicVariation > 0) {
				n.setDynamic(n.getDynamic() + (int) (Math.random() * (dynamicVariation * 2) - dynamicVariation));
			}
		}
	}
	 ...
	 */
	public static void repeat(CPhrase cphrase, final double startLoc, final double endLoc) {
	 ...
	 */
	public static void repeat(CPhrase cphrase, final int times, final double startLoc, final double endLoc) {
		...
		while (enum1.hasMoreElements()) {
			Phrase nextPhrase = (Phrase) enum1.nextElement();
			repeat(nextPhrase, times, startLoc - cphrase.getStartTime(), endLoc - cphrase.getStartTime());
		}
	}
	 ...
	 *            location of the loop end in beats
	 */
	public static void repeat(Part part, final double startLoc, final double endLoc) {
	...
	public static void repeat(Part part, final int times) {
		...
		/*
		 * // Alternate logic: Repeats the phase rather than lengthens the
		 * phrase int initialLength = part.getPhraseList().size(); for (int t =
		 * 0; t < (times - 1); t++){ double initialEndTime = part.getEndTime();
		 * for(int i = 0; i < initialLength; i++) { Phrase phr =
		 * (Phrase)part.getPhraseList().elementAt(i); Phrase phrCopy =
		 * phr.copy(); phrCopy.setStartTime(initialEndTime +
	 ...
	 * @param double location of the loop end in beats
	 */
	public static void repeat(Part part, final int times, final double startLoc, final double endLoc) {
	...
	/**
	 * Places a duplicate of a section of a phrase immediately after the end of
	 * the section.
	 * 
	 * <P>
	 ...
	 * @param startLoc
	 *            double describing the time of the beginning of the repeated
	 *            section, in crotchets.
	 * @param endLoc
	 *            double describing the time of the end of the repeated section,
	 *            in crotchets.
	 */
	public static void repeat(Phrase phrase, final double startLoc, final double endLoc) {
	 ...
	 *            Phrase to be repeated
	 * @param n
	 *            integer representing the number of repeats, 1 will leave the
	 *            phrase unchanged, 2 a single added repetitions, and so forth
	 */
	 ...
	 *            Phrase with section to be repeated
	 * @param times
	 *            integer representing the number of repeats
	 * @param startLoc
	 *            double describing the time of the beginning of the repeated
	 *            section, in crotchets.
	 * @param endLoc
	...
	public static void repeat(Phrase phrase, int times, double startLoc, double endLoc) {
		...
		for (beforeCount = 0; beforeCount < phrase.size()
				&& beatCounter + phrase.getNote(beforeCount).getRhythmValue() <= endLoc; beforeCount++) {
		...
		if (beforeCount + 1 < phrase.size()) { // make sure we haven't gone
												// through all notes
		...
		// if(startLoc >= 0.0 && phrase.getStartTime() > startLoc)
	 ...
	 *            - The CPhrase to be modified.
	 * @param amount
	 *            - The dynamic change possible either side of the curent
	 *            dynamic.
	 */
	...
	/**
	 * Randomly adjusts Note dynamic values to create uneven loudness. This
	 * process only effects the dynamic value of the notes in the phrase.
	 * <P>
	 * If <CODE>phrase</CODE> is null then this method does nothing.
	 * 
	 * @param phrase
	 ...
	 * 
	 * @param cphrase
	 *            -The CPhrase that contains phrases to be shuffled
	 */
	 ...
	 * 
	 * @param part
	 *            -The Part that contains phrases to be shuffled
	 */
	 ...
	 * 
	 * @param score
	 *            -The Score that contains phrases to be shuffled
	 */
	 ...
	 * having thier duration extended to 100% of the rhythm value. This
	 * algorithm assumes notes are arranged in the standard monophonic series
	 * used by jMusic. Notes with durations longer than rhythm values will
	...
	/**
	 * All descending sequences of notes are slured by having thier duration
	 * extended to 100% of the rhythm value. This algorithm assumes notes are
	 * arranged in the standard monophonic series used by jMusic. Notes with
	 * durations longer than rhythm values will produce unintended results.
	 * 
	 * @param Phrase
	 *            The phrase to be modified.
	 */
	...
	/**
	 * All descending sequences of the number of notes or more are slured by
	 * having the duration of all but the last note extended to 100% of the
	 * rhythm value. This algorithm assumes notes are arranged in the standard
	 * monophonic series used by jMusic. Notes with durations longer than rhythm
	...
	/**
	 * All ascending sequences of notes are slured by having thier duration
	 * extended to 100% of the rhythm value. This algorithm assumes notes are
	 * arranged in the standard monophonic series used by jMusic. Notes with
	 * durations longer than rhythm values will produce unintended results.
	 * 
	 ...
	 * stereo spectrum. This process only effects the pan value of the notes in
	 * the cphrase.
	 * <P>
	 * If <CODE>cphrase</CODE> is null then this method does nothing.
	 * 
	...
	/**
	 * Joins consecutive notes in each <CODE>CPhrase</CODE> that have the same
	 * pitch, creating one longer note. This is simmilar to the musical function
	 * of a tie. All note in the phrase meeting the conditions are effected.
	...
	/**
	 * Joins consecutive notes in each <CODE>phrase</CODE> that have the same
	 * pitch, creating one longer note. This is simmilar to the musical function
	 * of a tie. All note in the phrase meeting the conditions are effected.
	 * This modification may reduce the overall note count.
	 * 
	...
	/**
	 * Joins consecutive notes in the <CODE>phrase</CODE> that have the same
	 * pitch, creating one longer note. This is simmilar to the musical function
	 * of a tie. All note in the phrase meeting the conditions are effected.
	 * This modification may reduce the overall note count.
	 * 
	 ...
	 * note. This is simmilar to the musical function of a tie. This
	 ...
	 * @param transpose
	 *            integer describing the amount to transpose in semitones
	 */
	 ...
	 * @param key
	 *            the chromatic note to be used as the rooth of the mode. i.e.,
	 *            0 = C, 1 = C# etc.
	 */
	 ...
	 *            the amount to transpose in semitones
	 * @param mode
	 *            the scale to use for the transposition (the JMC has some scale
	 *            constants)
	 * @param key
	 *            the chromatic note to be used as the root of the mode. i.e., 0
	 *            = C, 1 = C# etc.
	 */
	...
	/**
	 * Transpose the part up or down in semitone steps. This transposition is
	 * chromatic, not diatonic.
	 * 
	 * @param part
	 *            the Part to be transposed
	 * @param transposition
	 *            the number of semitone steps to shift the pitch
	 */
	 ...
	 * values from note to note. This process only effects the duration
