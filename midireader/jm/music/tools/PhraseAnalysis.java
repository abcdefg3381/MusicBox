 ...
 * <LI>Bulk Statistics:
 * <OL>
 * Methods that call multiple other methods, and return their statistics in one
 * group. They never generate any new statistics themselves; every value in the
 * group can be accessed individually using the other methods in this class. The
 * method <CODE>getAllStatistics</CODE> is the only bulk statistic; it returns
 * all the feature statistics.
 * </OL>
 * </LI>
 * 
 * <LI><A NAME="Feature Statistics">Feature Statistics</A>:
 * <OL>
 * Methods that return double values in the range of 0 to 1. Usually these take
 * the form of a percentage measuring the degree to which the feature is
 * present, compared with the maximum value possible for the feature. <br>
 * Many feature statistics rely on the accessory statistics to generate values
 ...
 * <OL>
 * Methods that return more direct statistics, such as the number of distinct
 * pitches in a melody. These are generally more useful than the feature
 * statistics. All of the accessory statistics are used by at least one of the
 * feature statistics.
 * </OL>
 * </LI>
 * </LI>
 ...
 * <UL>
 * <LI>{@link #getAllStatistics}</LI>
 * </UL>
 * 
 * <P>
 * The feature statistics include:
 * <UL>
 * <LI>{@link #noteDensity}</LI>
 ...
 * 
 * <P>
 * For clarification, here are definitions of terms used in the method
 * descriptions.
 * <P>
 * <A NAME="Note">Note</A>
 * <UL>
 * Unless otherwise specified the term 'note' or 'notes' do not include rests.
 * This does not apply to the JMusic object Note, which will always be
 * capitalised.
 * </UL>
 * <P>
 * <A NAME="Quantum">Quantum duration</A>
 * <UL>
 * For a quantised melody, all rhythm values in the melody must be multiples of
 * the quantum duration. A note which has a rhythm value four times the quantum
 * duration can be described as being four quanta long.
 * </UL>
 * 
 * @author Adam Kirby
...
public final class PhraseAnalysis {
	 ...
	 * 
	 * <P>
	 * These intervals are measured in semitones and should not be greater than
	 * an octave as these are automatically considered unacceptable. Descending
	 * intervals are treated as their ascending counterparts, so all values
	 * returned by this array should be greater than zero.
	 * 
	 * <P>
	 * This constant is used by the {@link #dissonance dissonance} and
	 * {@link #checkDissonance checkDissonance} methods.
	 */
	...
	/**
	 * Array of Strings giving the identifier number and descriptive name of
	 * each feature as returned by the {@link #getAllStatistics
	 * getAllStatistics} methods.
	 */
	public static final String[] featureNames = { "01 - Pitch Variety", "02 - Pitch Range", "03 - Key Centeredness",
			"04 - Tonal Deviation", "05 - Dissonance", "06 - Overall Pitch Direction", "07 - Melodic Direction Stability",
			"08 - Pitch Movement By Tonal Step", "09 - Leap Compensation", "10 - Climax Strength", "11 - Climax Position",
			"12 - Climax Tonality", "13 - Note Density", "14 - Rest Density", "15 - Rhythmic Variety", "16 - Rhythmic Range",
			"17 - Syncopation", "18 - Repeated Pitch Density", "19 - Repeated Rhythmic Value Density",
			"20 - Repeated Pitch Patterns Of Three", "21 - Repeated Pitch Patterns Of Four",
			"22 - Repeated Rhythm Patterns Of Three", "23 - Repeated Rhythm Patterns Of Four" };
	 ...
	 * indicates a interval with a rest by adding this constant to the size of
	 * the interval.
	 * 
	 ...
	 */
	public static final int INTERVAL_WITH_REST = (Note.MAX_PITCH - Note.MIN_PITCH) * 2 + 1;
	 ...
	 * 
	 * <P>
	 * Many methods of this class require a scale of this form as parameters. It
	 * is suggested that either this constant or the <CODE>MINOR_SCALE</CODE>
	 ...
	 * suitably large enough that it would be a maximum for a reasonable melody.
	 * 
	 * <P>
	 * Within this class, this constant is only used by the
	 * {@link #pitchRangePerSpan pitchRangePerSpan} method.
	 * 
	 * <P>
	 * As such, this constant provides little benefit except to implementations
	 * that use the <CODE>pitchRangePerSpan</CODE> method.
	 */
	 ...
	 * value divided by the shortest. This maximum is not an actual maximum, but
	 * a value determined by those developing these methods as suitably large
	 * enough suitably large enough that it would be a maximum for a reasonable
	 * melody.
	 * 
	 * <P>
	 * Within this class, this constant is only used by the
	 * {@link #rhythmRangePerSpan rhythmRangePerSpan} method.
	 * 
	 * <P>
	 * As such, this constant provides little benefit except to implementations
	 * that use the <CODE>rhythmRangePerSpan</CODE> method.
	 */
	 ...
	 * <P>
	 * Many methods of this class require a scale of this form as parameters. It
	 * is suggested that either this constant or the <CODE>MAJOR_SCALE</CODE>
	 * constant be passed to these methods, depending on the key of the melody
	 * to be analysed.
	 */
	 ...
	 * 
	 * <P>
	 * This constant is used by the {@link #primaryQuantumCount
	...
	/**
	 * Integer constant describing the number of semitone intervals in any given
	 * octave.
	 */
	 ...
	 * is an interval whose absolute size is equal to or greater than
	 * {@link #BIG_JUMP_INTERVAL}.
	 * 
	 * @param phrase
	...
	/**
	 * Returns the number of big jumps in the specified Phrase. A big jump is an
	 * interval whose absolute size is equal to or greater than
	 * {@link #BIG_JUMP_INTERVAL}.
	 * 
	 * @param phrase
	 *            Phrase to be analysed
	 ...
	 * greater than {@link #BIG_JUMP_INTERVAL}. A step back is a interval in the
	 * opposite direction whose size is at least one semitone and less than or
	 * equal to {@link #BIG_JUMP_INTERVAL}.
	 * 
	 * @param noteArray
	 *            Note array to be analysed
	 * @return the number of big jumps
	 */
	public static int bigJumpFollowedByStepBackCount(final Note[] noteArray) {
		...
		if (intervalArray.length > 0) {
			...
			for (int i = 1; i < intervalArray.length - 1; i++) {
				intervalArray[i] = removeRestMarker(intervalArray[i]);
				if ((intervalArray[i - 1] >= BIG_JUMP_INTERVAL && intervalArray[i] < 0 && intervalArray[i] >= 0 - BIG_JUMP_INTERVAL)
						|| (intervalArray[i - 1] <= 0 - BIG_JUMP_INTERVAL && intervalArray[i] > 0 && intervalArray[i] <= BIG_JUMP_INTERVAL)) {
					count++;
				}
			}
			return count;
	 ...
	 */
	public static double climaxPosition(final Note[] noteArray) throws NoteListException {
	 ...
	 * @return double describing the position of the climax
	 */
	public static double climaxPosition(final Phrase phrase) throws NoteListException {
	 ...
	 * 
	 * @param noteArray
	 *            Note array to be examined
	 * @return double describing the climatic strength.
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Note
	 *                array
	 */
	public static double climaxStrength(final Note[] noteArray) throws NoteListException {
		int count = 0;
		int highestPitch = Note.MIN_PITCH;
		int currentPitch;

		for (Note element : noteArray) {
			currentPitch = element.getPitch();
			if (currentPitch != Pitches.REST) {
				if (currentPitch > highestPitch) {
					highestPitch = currentPitch;
					count = 1;
	 ...
	 * 
	 * @param Phrase
	 *            Phrase to be examined
	 * @return double describing the climatic strength.
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static double climaxStrength(final Phrase phrase) throws NoteListException {
	 ...
	 * </TR>
	 * <TR>
	 * <TH>Tonic or Dominant</TH>
	 * <TH>0.0</TH>
	 * </TR>
	 * <TR>
	 * <TH>Other scale note</TH>
	 * <TH>0.5</TH>
	 * </TR>
	 * <TR>
	 * <TH>Non-scale note</TH>
	 * <TH>1.0</TH>
	 * </TR>
	 * </TABLE>
	 * </CENTER>
	 * 
	 * <P>
	 * How the pitch rates depends on the scale of the melody. This information
	 * is passed to this method through an integer describing the tonic, and a
	 * integer array describing the scale.
	 * 
	 * @param noteArray
	 *            Note array to be analysed
	 * @param tonic
	 *            integer representing the pitch of the tonic in semitones,
	 *            ranging from 0 (C natural) to 11 (B flat).
	 * @param scale
	 *            array of integers describing the scale notes in the key of the
	 *            melody. If the key is major use the {@link #MAJOR_SCALE}
	 *            constant of this class; if harmonic minor use
	 *            {@link #MINOR_SCALE}. Arrays representing other keys can also
	 *            be created but they must fit the criteria outlined in the
	 *            descriptions of those constants.
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Note
	 *                array
	 */
	 ...
	 * is the last highest pitch, and given that pitch this method will return a
	 ...
	 * 
	 * <P>
	 * If there are no note to next note intervals in the melody, this method
	 * returns 0 as there are no consecutive identical pitches.
	 * 
	 * @param noteArray
	 *            Note array to be analysed
	 * @return integer describing the number of intervals with a size of 0
	 *         semitones.
	 */
	public static int consecutiveIdenticalPitches(final Note[] noteArray) {
		...
		for (int interval : intervals) {
			if (interval == 0 || interval == INTERVAL_WITH_REST) {
				count++;
			}
		}
		return count;
	 ...
	 * 
	 * <P>
	 * If there are no Notes in the melody, this method returns 0 as ther are no
	 * consecutve indentical rhythm values.
	 * 
	 * @param phrase
	 *            Phrase to be analysed
	 * @return integer describing the number of intervals with a size of 0
	 ...
	 * 
	 * <P>
	 * If the Note array has no intervals, it is considered to have no
	 ...
	 * 
	 * <P>
	 * If the Phrase has no intervals, it is considered to have no dissonance,
	 ...
	 * 
	 * @param noteArray
	 *            array of Note objects to be examined
	 * @return integer describing the number of distinct pitches in the Note
	 *         array
	 */
	...
	public static int distinctRhythmCount(final Note[] noteArray) {
		...
		double rhythm;
		for (Note element : noteArray) {
			if (element.getPitch() <= Pitches.REST) {
				rhythm = element.getRhythmValue();
	 ...
	 * 
	 * <P>
	 * The keys of the hashtable contain a String describing the feature along
	 * with a numeric identification code for that feature.
	 * 
	 * <P>
	 * The values are Strings representing the value returned by that feature:
	 * either the double returned, or a description of the exception thrown.
	 * 
	 * @param noteList
	 *            the Note array to be analysed
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0.
	 * @param tonic
	 *            integer representing the pitch of the tonic in semitones,
	 *            ranging from 0 (C natural) to 11 (B flat).
	 * @param scale
	 ...
	 *            be created but they must fit the criteria outlined in the
	 *            descriptions of those constants.
	 * @return hashtable whose keys contain a String describing the feature
	 *         statistics, and whose corresponding values are the doubles
	 *         returned by each statistic converted to a String.
	 */
	public static Hashtable getAllStatistics(final Note[] noteList, final double duration, final int tonic, final int[] scale) {
		String[] statistics = getAllStatisticsAsStrings(noteList, duration, tonic, scale);
	 ...
	 *         returned by each statistic converted to a String.
	 */
	public static Hashtable getAllStatistics(final Phrase phrase, final double duration, final int tonic, final int[] scale) {
		return getAllStatistics(phrase.getNoteArray(), duration, tonic, scale);
	}

	public static double[] getAllStatisticsAsDoubles(final Note[] noteArray, final double duration, final int tonic,
			final int[] scale) {
	...
	public static double[] getAllStatisticsAsDoubles(final Phrase phrase, final double duration, final int tonic,
			final int[] scale) {
		return getAllStatisticsAsDoubles(phrase.getNoteArray(), duration, tonic, scale);
	}

	public static String[] getAllStatisticsAsStrings(final Note[] noteArray, final double duration, final int tonic,
			final int[] scale) {
		...
		try {
			statistics[2] = Double.toString(keyCenteredness(noteArray, duration, tonic));
		} catch (NoteListException e) {
		...
		try {
			statistics[3] = Double.toString(tonalDeviation(noteArray, duration, tonic, scale));
		} catch (NoteListException e) {
		...
		try {
			statistics[6] = Double.toString(melodicDirectionStability(noteArray));
		} catch (NoteListException e) {
			statistics[6] = NOTELIST_EXCEPTION_STRING;
		}
		try {
			statistics[7] = Double.toString(movementByStep(noteArray, tonic, scale));
		} catch (NoteListException e) {
		...
		try {
			statistics[11] = Double.toString(climaxTonality(noteArray, tonic, scale));
		} catch (NoteListException e) {
		...
		try {
			statistics[18] = Double.toString(repeatedRhythmicValueDensity(noteArray));
		} catch (NoteListException e) {
			statistics[18] = NOTELIST_EXCEPTION_STRING;
		}
		try {
			statistics[19] = Double.toString(repeatedPitchPatterns(noteArray, 3));
		} catch (NoteListException e) {
			statistics[19] = NOTELIST_EXCEPTION_STRING;
		}
		try {
			statistics[20] = Double.toString(repeatedPitchPatterns(noteArray, 4));
		} catch (NoteListException e) {
			statistics[20] = NOTELIST_EXCEPTION_STRING;
		}
		try {
			statistics[21] = Double.toString(repeatedRhythmPatterns(noteArray, 3));
		} catch (NoteListException e) {
			statistics[21] = NOTELIST_EXCEPTION_STRING;
		}
		try {
			statistics[22] = Double.toString(repeatedRhythmPatterns(noteArray, 4));
		} catch (NoteListException e) {
	...
	public static String[] getAllStatisticsAsStrings(final Phrase phrase, final double duration, final int tonic,
			final int[] scale) {
		return getAllStatisticsAsStrings(phrase.getNoteArray(), duration, tonic, scale);
	}
	...
	public static int intervalSemitoneCount(final Note[] noteArray) {
		int count = 0;
		int[] intervalArray = pitchIntervals(noteArray);
		for (int element : intervalArray) {
			count += Math.abs(removeRestMarker(element));
		}
		return count;
	}
	 ...
	 * 
	 * @param patterns
	 *            array of double arrays storing the patterns
	 * @param match
	 *            double array to search for
	 * @param n
	 *            the number of elements of <CODE>patterns</CODE> to search
	 * @return true if pattern is found; false otherwise
	 */
	private static boolean isAlreadyMatched(final double[][] patterns, final double[] match, final int n) {
	 ...
	 * 
	 * @param patterns
	 *            array of integer arrays storing the patterns
	 * @param match
	 *            integer array to search for
	 * @param n
	 *            the number of elements of <CODE>patterns</CODE> to search
	 * @return true if pattern is found; false otherwise
	 */
	private static boolean isAlreadyMatched(final int[][] patterns, final int[] match, final int n) {
		search: for (int i = 0; i < n; i++) {
			for (int j = 0; j < match.length; j++) {
				if (patterns[i][j] != match[j]) {
					continue search;
	 ...
	 * 
	 * @param element
	 *            double to search forinteger to search for
	 * @param array
	 *            array of doubles to be searched for match
	 * @param n
	 *            integer indicating where to stop the search in the array
	 * @return true if found; false otherwise
	 */
	private static boolean isElementOf(final double element, final double[] array, final int n) {
		for (int i = 0; i < n; i++) {
			if (array[i] == element) {
				return true;
			}
	 ...
	 * 
	 * @param array
	 *            array of integers to be searched for match
	 * @param element
	 *            integer to search for
	 * @param n
	 *            integer indicating where to/ stop the search in the array
	 * @return true if found; false otherwise
	 */
	private static boolean isElementOf(final int element, final int[] array, final int n) {
		for (int i = 0; i < n; i++) {
			if (array[i] == element) {
				return true;
			}
	 ...
	 * 
	 * @param noteArray
	 *            array of Notes to be tested.
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0, otherwise an exception
	 *            is thrown.
	 ...
	 * @see Phrase
	 */
	public static boolean isQuantised(final Note[] noteArray, final double duration) throws QuantisationException {
		if (duration > 0) {
			for (Note element : noteArray) {
				if (element.getRhythmValue() % duration != 0.0) {
					return false;
				}
			}
			return true;
	 ...
	 * @see Phrase
	 */
	public static boolean isQuantised(final Phrase phrase, final double duration) throws QuantisationException {
	...
	/**
	 * Returns a double expressing the total duration of all primary notes, over
	 * the length of the melody. Primary notes are either tonic or dominant.
	 * 
	 * @param noteArray
	 *            array of Notes to be analysed
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0.
	 * @param tonic
	 *            integer representing the pitch of the tonic in semitones,
	 *            ranging from 0 (C natural) to 11 (B flat).
	 * @return double describing the fraction of primary quanta per quantum in
	 *         the melody
	 * @exception NoteListException
	 *                if the number of quanta in the Phrase is zero
	 * @exception QuantisationException
	 *                if the notes' rhythm values are not multiples of quantum
	 *                duration, or if quantum duration is less than or equal to
	 *                zero
	 */
	public static double keyCenteredness(final Note[] noteArray, final double duration, final int tonic)
			throws QuantisationException, NoteListException {
		int quanta = quantumCount(noteArray, duration);
		if (quanta > 0) {
			return primaryQuantumCount(noteArray, duration, tonic) / (double) quanta;
		} else {
	 ...
	 *                zero
	 */
	public static double keyCenteredness(final Phrase phrase, final double duration, final int tonic)
			throws QuantisationException, NoteListException {
	...
	public static double leapCompensation(Note[] noteArray) {
		...
		if (bigJumpCount != 0) {
			return (bigJumpCount - bigJumpFollowedByStepBackCount(noteArray)) / (double) bigJumpCount;
		} else {
			return 0.0;
		}
	}
	 ...
	 * 
	 * @param array
	 *            double array whose elements contain the potential pattern
	 * @param firstIndex
	 *            index to the start of the first pattern
	 * @param secondIndex
	 *            index to the start of the second pattern
	 * @param n
	 *            the number of element to match
	 * @return true if the first pattern is the same as the second; false
	 *         otherwise
	 */
	private static boolean matchPattern(final double[] array, final int firstIndex, final int secondIndex, final int n) {
		boolean flag = true;
		for (int i = 0; i < n; i++) {
	 ...
	 *         otherwise
	 */
	private static boolean matchPattern(final int[] array, final int firstIndex, final int secondIndex, final int n) {
		boolean flag = true;
		for (int i = 0; i < n; i++) {
			if (array[firstIndex + i] != array[secondIndex + i]) {
				flag = false;
	 ...
	 * @exception NoteListException
	 *                if there are less than three <A HREF="#Note">notes</A> in
	 *                the Note array
	 */
	public static double melodicDirectionStability(final Note[] noteArray) throws NoteListException {
		int intervalCount = intervalCount(noteArray);
		if (intervalCount - 1 != 0) {
			return sameDirectionIntervalCount(noteArray) / (double) intervalCount;
		} else {
	 ...
	 *                the Phrase
	 */
	public static double melodicDirectionStability(final Phrase phrase) throws NoteListException {
	 ...
	 * specified Note array. This indicates the smoothness of the melody. A high
	 ...
	 *                the Note array
	 */
	public static double movementByStep(final Note[] noteArray, final int tonic, final int[] scale) throws NoteListException {
		int intervalCount = intervalCount(noteArray);
		if (intervalCount > 0) {
			return stepIntervalCount(noteArray, tonic, scale) / (double) intervalCount;
		} else {
	 ...
	 *                the Phrase
	 */
	public static double movementByStep(final Phrase phrase, final int tonic, final int[] scale) throws NoteListException {
	 ...
	 * 
	 * <P>
	 * The key signature of the melody is specified by parameters passed to this
	 * method, namely an integer describing the tonic, and a integer array
	 ...
	 *            be created but they must fit the criteria outlined in the
	 *            descriptions of those constants.
	 * @return integer describing the number of non-scale notes in the specified
	 *         Note array
	 */
	public static int nonScaleQuantumCount(final Note[] noteArray, final double duration, final int tonic, final int[] scale)
		...
		if (isQuantised(noteArray, duration)) {
			...
			for (Note element : noteArray) {
				pitch = element.getPitch();
				if (pitch != Pitches.REST && !isElementOf(pitchToDegree(pitch, tonic), scale)) {
					count += (int) (element.getRhythmValue() / duration);
				}
			}
			return count;
		} else {
	 ...
	 *         Phrase
	 */
	public static int nonScaleQuantumCount(final Phrase phrase, final double duration, final int tonic, final int[] scale)
			throws QuantisationException {
		return nonScaleQuantumCount(phrase.getNoteArray(), duration, tonic, scale);
	}

	/**
	 * Returns the number of <A HREF="#Note">notes</A> in a specified
	 * {@link Note} array.
	 * 
	 * @param noteArray
	 *            Note array whose notes are to be counted
	 * @return integer describing the number of non-rest Notes in the Note
	 *         array.
	 */
	 ...
	 * 
	 * @param phrase
	 *            Phrase whose notes are to be counted
	 ...
	 * 
	 * @param noteArray
	 *            array of Notes to be examined
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0.
	 * @return double describing the fraction of notes per quantum in the Note
	 *         array
	 * @exception NoteListException
	 ...
	 *                zero
	 */
	public static double noteDensity(final Note[] noteArray, final double duration) throws NoteListException,
	 ...
	 *                zero
	 */
	public static double noteDensity(final Phrase phrase, final double duration) throws NoteListException,
			QuantisationException {
	 ...
	 * 
	 * <P>
	 * This feature checks the overall tendency of the melodic contour. A melody
	 * starting and finishing on the same note will score 0.5, higher indicates
	 * an overall rise, and lower scores indicate a descending contour.
	 * 
	 * @param noteArray
	 *            Note array to be analysed
	 ...
	 * unchanged. However, if the interval has 1 or more rests between its notes
	 * the integer is increased by the value of the {@link #INTERVAL_WITH_REST}
	 * constant.
	 * 
	 * <P>
	 * Given any value in the array returned you can determine if it represents
	 * an interval with 1 or more rests between its notes in the following
	 * manner. If the value is greater than the largest valid rising interval,
	 * namely ({@link Note#MAX_PITCH} - {@link Note#MIN_PITCH}), then it
	 * represents an interval with a rest. Otherwise it represents an interval
	 * without a rest.
	 * 
	 * <P>
	 * Given a value representing an interval with a rest, you can determine the
	 * size and direction of the interval by subtracting INTERVAL_WITH_REST from
	 * the value.
	 * 
	 * @param noteArray
	 *            Note array to be examined
	...
	public static int[] pitchIntervals(final Note[] noteArray) {
		...
		if (intervalCount > 0) {
			...
			while (noteArray[++index].getPitch() == Pitches.REST) {
			}
			...
			for (int i = 0; i < intervalArray.length; i++) {
				...
				// INTERVAL_WITH_REST
				while (currentPitch == Pitches.REST) {

					// Only make the change for the first rest
					if (noteArray[index - 1].getPitch() != Pitches.REST) {
	...
	/**
	 * Returns an array of integers describing the size of intervals between the
	 * <A HREF="#Note">notes</A> in the specified {@link Phrase}. If an interval
	 * has no rests between its notes the integer representing it is unchanged.
	 * However, if the interval has 1 or more rests between its notes the
	 * integer is increased by the value of the {@link #INTERVAL_WITH_REST}
	 * constant.
	 * 
	 * <P>
	 * Given any value in the array returned you can determine if it represents
	 ...
	 * 
	 * @param noteArray
	 *            Note array to be searched
	 * @param chunksize
	 *            The size of the pattern to search for
	 * @return integer describing the number of 'pitch interval' patterns
	 */
	public static int pitchPatternCount(final Note[] noteArray, final int chunkSize) {
		int count = 0;
		int[] intervalArray = pitchIntervals(noteArray);
		...
		if (intervalArray.length > chunkSize) {
			...
			for (int i = 0; i < intervalArray.length - chunkSize; i++) {
				...
				if (!isAlreadyMatched(patterns, match, index)) {
					for (int j = i + 1; j < intervalArray.length - chunkSize + 1; j++) {
	 ...
	 * @return integer describing the number of 'pitch interval' patterns
	 */
	public static int pitchPatternCount(final Phrase phrase, final int chunkSize) {
	 ...
	 * @param noteArray
	 *            Note array to be examined
	 * @return integer describing the difference between the highest and lowest
	 *         pitches
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Note
	 *                array
	 */
	public static int pitchRange(final Note[] noteArray) throws NoteListException {
		int highestPitch = Note.MIN_PITCH;
	 ...
	 * 
	 * @param phrase
	 *            Phrase whose notes are to be examined
	 * @return integer describing the difference between the highest and lowest
	 *         pitches
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static int pitchRange(final Phrase phrase) throws NoteListException {
	 ...
	 * 
	 * <P>
	 * <CODE>MAX_PITCH_RANGE</CODE> is an arbitrary value that was useful for
	 ...
	 *                array
	 */
	public static double pitchRangePerSpan(final Note[] noteArray) throws NoteListException {
	 ...
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static double pitchRangePerSpan(final Phrase phrase) throws NoteListException {
	...
	public static int pitchToDegree(int pitch, final int tonic) {
		...
		if (pitch < 0) {
			...
			// scale
			pitch += ((-pitch / SEMITONES_PER_OCTAVE) + 1) * SEMITONES_PER_OCTAVE;
		}
	 ...
	 * @param noteArray
	 *            array of Notes to be examined
	 * @return double describing the fraction of distinct pitches per note in
	 *         the Note array
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Note
	 *                array
	 */
	public static double pitchVariety(final Note[] noteArray) throws NoteListException {
	 ...
	 * 
	 * @param phrase
	 *            Phrase whose pitches are to be counted
	 * @return double describing the fraction of distinct pitches per note in
	 *         the Phrase
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static double pitchVariety(final Phrase phrase) throws NoteListException {
	 ...
	 *            integer representing the pitch of the tonic in semitones,
	 *            ranging from 0 (C natural) to 11 (B flat).
	 * @return integer describing the number of 'primary notes' in the specified
	 *         Note array
	 * @exception QuantisationException
	 *                if the Notes' rhythm values are not multiples of quantum
	 *                duration, or if quantum duration is less than or equal to
	 *                zero
	 */
	public static int primaryQuantumCount(final Note[] noteArray, final double duration, final int tonic)
			throws QuantisationException {
		if (isQuantised(noteArray, duration)) {
			int count = 0;
			int pitch;
			...
			for (Note element : noteArray) {
				pitch = element.getPitch();
				if (pitch == Pitches.REST || isElementOf(pitchToDegree(pitch, tonic), PRIMARY_NOTES)) {
					count += (int) (element.getRhythmValue() / duration);
				}
			}
			return count;
	 ...
	 *                zero
	 */
	public static int primaryQuantumCount(final Phrase phrase, final double duration, final int tonic)
			throws QuantisationException {
		return primaryQuantumCount(phrase.getNoteArray(), duration, tonic);
	}

	/**
	 * Returns the number of <A HREF="#Quantum">quanta</A> in a specified
	 * {@link Note} array.
	 * 
	 * @param noteArray
	 *            Note array whose quanta are to be counted
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This value must be greater than 0.
	 * @return integer describing the length of the Note array measured in
	 *         quanta.
	 * @exception QuantisationException
	 *                if the Notes' rhythm values are not multiples of the
	 *                specified quantum duration, or if quantum duration is less
	 *                than or equal to zero
	 */
	public static int quantumCount(final Note[] noteArray, final double duration) throws QuantisationException {
		if (isQuantised(noteArray, duration)) {
			int count = 0;
			for (Note element : noteArray) {
				count += (int) (element.getRhythmValue() / duration);
	 ...
	 * 
	 * @param phrase
	 *            Phrase whose quanta are to be counted
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This value must be greater than 0.
	 ...
	 *                than or equal to zero
	 */
	public static int quantumCount(final Phrase phrase, final double duration) throws QuantisationException {
	 ...
	 * 
	 * <P>
	 * If the pitch is in the array {@link GOOD_INTERVALS} this method returns
	 * 0, if in {@link BAD_INTERVALS} 2, otherwise 1.
	 * 
	 * @param i
	 *            Pitch to be rated, relative to the tonic
	 * @return 0 if good, 2 if bad, 1 otherwise
	 */
	private static int rateDissonance(final int i) {
		for (int element : GOOD_INTERVALS) {
			if (i == element) {
				return 0;
			}
		}
		for (int element : BAD_INTERVALS) {
			if (i == element) {
				return 2;
			}
		}
	 ...
	 * 
	 * @param interval
	 *            integer representing the interval
	 * @return interval with rest marker removed (if present)
	 */
	public static int removeRestMarker(final int interval) {
		return (interval > Note.MAX_PITCH - Note.MIN_PITCH) ? interval - INTERVAL_WITH_REST : interval;
	}
	 ...
	 *                the Note array
	 */
	public static double repeatedPitchDensity(final Note[] noteArray) throws NoteListException {
		int intervalCount = intervalCount(noteArray);
		if (intervalCount != 0) {
			return consecutiveIdenticalPitches(noteArray) / (double) intervalCount;
		} else {
			throw new NoteListException("The melody should contain at least" + " two notes.");
		}
	}
	 ...
	 *                the Phrase
	 */
	public static double repeatedPitchDensity(final Phrase phrase) throws NoteListException {
	 ...
	 *            Note array to be analysed.
	 * @param chunkSize
	 *            integer describing the size of patterns to count
	 * @exception NoteListException
	 *                if the melody does not have more <A HREF="#Note">
	 *                notes</A> than the size of the patterns searched for
	 */
	public static double repeatedPitchPatterns(final Note[] noteArray, final int chunkSize) throws NoteListException {
		int possiblePatterns = intervalCount(noteArray) - chunkSize;
		if (possiblePatterns > 0) {
			return pitchPatternCount(noteArray, chunkSize) / (double) possiblePatterns;
		} else {
			throw new NoteListException("The melody must contain more intervals" + " than the size of the pattern being"
					+ " searched for.");
		}
	}
	 ...
	 *                notes</A> than the size of the patterns searched for
	 */
	public static double repeatedPitchPatterns(final Phrase phrase, final int chunkSize) throws NoteListException {
	 ...
	 *                the Note array
	 */
	public static double repeatedRhythmicValueDensity(final Note[] noteArray) throws NoteListException {
		int intervalCount = intervalCount(noteArray);
		if (intervalCount != 0) {
			return consecutiveIdenticalRhythms(noteArray) / (double) intervalCount;
		} else {
			throw new NoteListException("The melody should contain at least" + " two notes.");
		}
	}
	 ...
	 *                the Phrase
	 */
	public static double repeatedRhythmicValueDensity(final Phrase phrase) throws NoteListException {
	 ...
	 *                notes</A> than the size of the patterns searched for
	 */
	public static double repeatedRhythmPatterns(final Note[] noteArray, final int chunkSize) throws NoteListException {
		int possiblePatterns = intervalCount(noteArray) - chunkSize;
		if (possiblePatterns > 0) {
			return rhythmPatternCount(noteArray, chunkSize) / (double) possiblePatterns;
		} else {
			throw new NoteListException("The melody must contain more" + " intervals than the size of the"
	 ...
	 *                notes</A> than the size of the patterns searched for
	 */
	public static double repeatedRhythmPatterns(final Phrase phrase, final int chunkSize) throws NoteListException {
		return repeatedRhythmPatterns(phrase.getNoteArray(), chunkSize);
	}

	/**
	 * Returns a double expressing the ratio between the number of rest <A
	 * HREF="#Quantum">quanta</A> and the number of quanta in a Note array. That
	 * is, the percentage of the time taken up by rests.
	 * 
	 * @param noteArray
	 *            array of Note objects to be examined
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0.
	 * @return double describing the fraction of silent quanta per quantum in
	 *         the Note array
	 * @exception NoteListException
	 ...
	 *                zero
	 */
	public static double restDensity(final Note[] noteArray, final double duration) throws NoteListException,
			QuantisationException {
		int quanta = quantumCount(noteArray, duration);
		if (quanta != 0) {
	...
	/**
	 * Returns a double expressing the ratio between the number of rest <A
	 * HREF="#Quantum">quanta</A> and the number of quanta in a Phrase. That is,
	 * the percentage of the time taken up by rests.
	 * 
	 * @param phrase
	 *            Phrase to be examined
	 * @param duration
	 ...
	 *                zero
	 */
	public static double restDensity(final Phrase phrase, final double duration) throws NoteListException,
			QuantisationException {
	 ...
	 * rests. Silent quanta do not include the quanta representing notes with a
	 * dynamic, or volume, of zero.
	 * 
	 * @param noteArray
	 *            Note array whose silent quanta are to be measured
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0.
	 ...
	 *                zero
	 */
	public static int restQuantumCount(final Note[] noteArray, final double duration) throws QuantisationException {
		if (isQuantised(noteArray, duration)) {
			int count = 0;
			for (Note element : noteArray) {
				if (element.getPitch() == Pitches.REST) {
	 ...
	 * 
	 * @param phrase
	 *            Phrase whose silent quanta are to be measured
	 * @param duration
	 *            double representing the length of a quantum, where 1 is a
	 *            crotchet. This must be greater than 0.
	 * @return integer describing the number of silent quanta in phrase
	 ...
	 *                zero
	 */
	public static int restQuantumCount(final Phrase phrase, final double duration) throws QuantisationException {
	 ...
	 * intervals in the specified {@link Note} array. The absolute value for any
	 ...
	 * 
	 * <P>
	 * Intervals involving any trailing rests are ignored. So the second note of
	 * the final interval is guaranteed to not be a rest.
	 * 
	 * @param phrase
	 *            Phrase to be examined
	 * @return array of integers describing the intervals
	...
	public static double[] rhythmIntervals(final Note[] noteArray) {
		...
		for (int i = noteArray.length - 1; noteArray[i].getPitch() == Pitches.REST && i > -1; i--) {
			intervalCount--;
		}
		...
		if (intervalCount > 0) {
			...
			for (int i = 0; i < intervalArray.length; i++) {
				intervalArray[i] = noteArray[i + 1].getRhythmValue() / noteArray[i].getRhythmValue();

				// Indicate a rest with a negative sign
				if (noteArray[i].getPitch() == Pitches.REST) {
					intervalArray[i] *= -1;
				}
			}
			return intervalArray;
	...
	public static int rhythmPatternCount(final Note[] noteArray, final int chunkSize) {
		int count = 0;
	 ...
	 * @return integer describing the number of 'rhythm interval' patterns
	 */
	public static int rhythmPatternCount(final Phrase phrase, final int chunksize) {
		return rhythmPatternCount(phrase.getNoteArray(), chunksize);
	}

	/**
	 * Returns the value of the longest rhythm value, divided by the shortest in
	 * the specified Note array.
	 * 
	 * @param phrase
	 *            Phrase whose notes are to be examined
	 * @return double describing the longest rhythm value, divided by the
	 *         shortest
	 * @exception NoteListException
	 *                if there are no <A HREF="#Note">notes</A> in the Note
	 *                array
	 */
	public static double rhythmRange(final Note[] noteArray) throws NoteListException {
		double longestRhythm = Note.MIN_RHYTHM_VALUE;
		double shortestRhythm = Note.MAX_RHYTHM_VALUE;
		double currentRhythm;

		for (Note element : noteArray) {
			currentRhythm = element.getRhythmValue();
			if (element.getPitch() != Pitches.REST) {
		...
		if (longestRhythm != Note.MIN_RHYTHM_VALUE && shortestRhythm != Note.MAX_RHYTHM_VALUE) {
	 ...
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static double rhythmRange(final Phrase phrase) throws NoteListException {
	 ...
	 * 
	 * <P>
	 * <CODE>MAX_RHYTHM_RANGE</CODE> is an arbitrary value that was useful for
	 * the developers of this method. If this arbitrary constant means nothing
	 * to your implementation consider using the {@link #rhythmRange
	 ...
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static double rhythmRangePerSpan(final Note[] noteArray) throws NoteListException {
	 ...
	 *                if there are no <A HREF="#Note">notes</A> in the Phrase
	 */
	public static double rhythmRangePerSpan(final Phrase phrase) throws NoteListException {
	...
	public static double rhythmValueCount(final Note[] noteArray) {
		double count = 0;
		for (Note element : noteArray) {
			count = count + element.getRhythmValue();
		}
		return count;
	}
	 ...
	 * @param noteArray
	 *            Note array to be analysed
	 * @return Integer describing the count of consecutive intervals in the same
	 *         direction
	 */
	public static int sameDirectionIntervalCount(final Note[] noteArray) {
		...
		if (intervalArray.length > 0) {
			...
			for (int i = 1; i < intervalArray.length; i++) {
				...
				if ((intervalArray[i] > 0 && intervalArray[i - 1] > 0) || (intervalArray[i] == 0 && intervalArray[i - 1] == 0)
	 ...
	 * @return double describing the movement by step
	 */
	public static int stepIntervalCount(final Note[] noteArray, final int tonic, final int[] scale) {
		int intervalCount = intervalCount(noteArray);
		if (intervalCount > 0) {
			int index = -1;
			int count = 0;
			...
			for (int i = 0; i < intervalCount; i++) {
				...
				if (Math.abs(currentPitch - previousPitch) < 3 && isElementOf(pitchToDegree(currentPitch, tonic), scale)
						&& isElementOf(pitchToDegree(previousPitch, tonic), scale)) {
					count++;
				}
				previousPitch = currentPitch;
			}
	...
	/**
	 * Returns a count of all the step intervals in the specified Phrase. A step
	 * interval is an interval between two scale <A HREF="#Note">notes</A>,
	 * whose difference is at most one tone.
	 * 
	 * <P>
	 * The key signature of the melody is specified by parameters passed to this
	 ...
	 * @return double describing the movement by step
	 */
	public static int stepIntervalCount(final Phrase phrase, final int tonic, final int[] scale) {
	 ...
	 * 
	 * <P>
	 * In simple time a beat is considered to be crotchet in length. This
	...
	public static double syncopation(final Note[] noteArray) {
		...
		for (Note element : noteArray) {
			rhythmValue = element.getRhythmValue();
			if (rhythmValue >= 1.0 && element.getPitch() != Pitches.REST && position % 1.0 != 0.0) {
				count++;
			}
	 ...
	 *            be created but they must fit the criteria outlined in the
	 *            descriptions of those constants.
	 * @return double describing the fraction of non-scale quanta per quantum in
	 *         the Note array
	 * @exception NoteListException
	 *                if the number of quanta in the Phrase is zero
	 * @exception QuantisationException
	 ...
	 *                zero
	 */
	public static double tonalDeviation(final Note[] noteArray, final double duration, final int tonic, final int[] scale)
			throws NoteListException, QuantisationException {
		int quanta = quantumCount(noteArray, duration);
		if (quanta != 0) {
			return nonScaleQuantumCount(noteArray, duration, tonic, scale) / (double) quanta;
		} else {
			throw new NoteListException("The length of the melody should be" + " greater than 0.");
		}
	}
	 ...
	 *                zero
	 */
	public static double tonalDeviation(final Phrase phrase, final double duration, final int tonic, final int[] scale)
			throws NoteListException, QuantisationException {
