...
public final class MidiUtil implements JMC {
	...
	/**
	 * Returns the time of a MIDI event by finding its end event and calculating
	 * the time between the start and end events
	 * 
	 * @param Node
	 *            node
	 */
	public static double getEndEvt(short pitch, Vector evtList, int index) {
		...
		for (; index < evtList.size(); index++) {
			...
			case 005:
				...
				if (noteOn.getPitch() == pitch && noteOn.getVelocity() == 0) {
