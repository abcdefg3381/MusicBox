...
public final class SMF implements JMC {
	...
	// ------------------------------------------
	/**
	 * Create a new empty SMF with default filetype and ppqn settings SMF
	 * objects are useful for retrieving statistical information about a MIDI
	 * file which has just been read or written.
	 */
	public SMF() {
	 ...
	 */
	public void print() {
		Enumeration enum1 = trackList.elements();
		while (enum1.hasMoreElements()) {
			Track track = (Track) enum1.nextElement();
			track.print();
		}
	}
	...
	private void writeTrackChunk(DataOutputStream odos, Track track) throws IOException {
		...
		Enumeration enum1 = track.getEvtList().elements();
		enum1 = track.getEvtList().elements();
		// At this stage Except that all events are NoteOn events
		while (enum1.hasMoreElements()) {
			Event evt = (Event) enum1.nextElement();
			evt.write(dos);
			if (DEBUG) {
				evt.print();
			}
