...
public class MidiSynth implements JMC, MetaEventListener {
	 ...
	 */
	protected static MidiEvent createCChangeEvent(int channel, int controlNum, int value, long tick)
	 ...
	 */
	protected static MidiEvent createNoteOffEvent(int channel, int pitch, int velocity, long tick)
			throws InvalidMidiDataException {
	 ...
	 * @param long tick is the time this event occurs
	 */
	protected static MidiEvent createNoteOnEvent(int channel, int pitch, int velocity, long tick)
			throws InvalidMidiDataException {
	...
	/**
	 * Invoked when a Sequencer has encountered and processed a MetaMessage in
	...
	protected void printSeqInfo(Sequence seq) {
		 ...
		 * (Sequence.SMPTE_24 == type) System.out.println("SMPTE 24 (24 fps)");
		 * else if (Sequence.SMPTE_25 == type)
		 * System.out.println("SMPTE 25 (25 fps)"); else if (Sequence.SMPTE_30
		 * == type) System.out.println("SMPTE 30 (30 fps)"); else if
		 * (Sequence.SMPTE_30DROP == type)
		 * System.out.println("SMPTE 30 Drop (29.97 fps)"); else
	...
	protected Sequence scoreToSeq(Score score) throws InvalidMidiDataException {
		...
		// System.out.println("jMusic MidiSynth notification: Score TempoEvent (BPM) = "
		// + score.getTempo());
		...
		while (parts.hasMoreElements()) {
			...
			// System.out.println("jMusic MidiSynth notification: Part TempoEvent (BPM) = "
			// + tempo);
			...
			while (phrases.hasMoreElements()) {
				...
				if (tempo != Phrase.DEFAULT_TEMPO) {
					m_currentTempo = tempo;
					// System.out.println("jMusic MidiSynth notification: Phrase TempoEvent (BPM) = "
					// + tempo);
				}
		...
		if (longestTime > 0.0 && longestTrack != null) {
			...
			MidiEvent evt = new MidiEvent(msg, (long) longestTime + 100); // leave
																			// some
																			// space
																			// for
																			// reverb
