 ...
 * sound data being worked on and the envelope itself is constructed in the
 * build() method.<br>
...
public class Envelope extends AudioObject implements JMC {
	...
	/*
	 * The index in the note's breakPoint array to use. See the Note class for
	 ...
	 * samples based on the envelope, it uses the note's amp env values.
	 * 
	 * @param ao
	 *            the single AudioObject to use as input.
	 */
	 ...
	 * 
	 * @param breakPoints
	 *            the data to construct the envelope from
	 * @param ao
	 *            the single AudioObject to use as input.
	 */
	 ...
	 * 
	 * @param graphPoints
	 *            the points to construct the envelope from
	 * @param ao
	 *            the single AudioObject to use as input.
	 */
	...
	/**
	 * An Envelope object can be used as a generator. This is a method to call
	 * to do this.
	 * 
	 * @param sampleRate
	 ...
	 *            the sampleRate for this AudioObject.
	 * @param notePointIndex
	 *            The note's break point envelope values to use.
	 */
	...
	// ----------------------------------------------
	/**
	 * This method used to convert double value arrays into a graphPoint array.
	 */
	private void breakPointsToGraphPoints(double[] breakPoints) {
		...
		for (int i = 0; i < (breakPoints.length / 2); i++) {
			graphPoints[i] = new EnvPoint((float) breakPoints[counter], (float) breakPoints[counter + 1]);
			counter += 2;
		}
	}
	...
	@Override
	public void build() {
		// System.out.println("samps = " + numOfSamples + " Graph points = " +
		// graphPoints.length);
		...
		if (this.graphPoints[0].x != -1.0) {
			for (int i = 0; i < this.graphPoints.length; i++) {
				this.graphPoints[i].X = ((int) ((numOfSamples) * this.graphPoints[i].x));
		...
		for (int i = 0; i < (this.graphPoints.length) - 1; i++) {
			float gradient =
					(this.graphPoints[i].y - this.graphPoints[i + 1].y) / (this.graphPoints[i].X - this.graphPoints[i + 1].X);
			float yintercept = this.graphPoints[i + 1].y - (gradient * this.graphPoints[i + 1].X);
			for (;;) {
				...
				this.graphShape[j] = (gradient * j) + yintercept;
	 ...
	 * positional value will be passed on unchanged.
	 * 
	 * @param input
	 *            input data
	 */
	...
	public int work(float[] buffer) throws AOException {
		...
		// process data
		if (primary) {
			// System.out.println("in primary. Graph size = " +
			// graphShape.length + " Postion = " + position);
