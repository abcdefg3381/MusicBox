 ...
 * * They provide a fast mechanism for seperating frequencies using a
 * mathamatical process known as a z-transform. This implementation is based on
 * the Chebyshev filter design in Steven W.Smith's excellent book
 * "The Scientist and Engineer's Guide to Digital Signal Processing". Band pass
 * and resonant filters can be created by combining law and high pass filters.
 * 
...
public final class Filter extends AudioObject {
	...
	/**
	 * Cutoff frequency for the filter as a percentage of the sampling rate
	 */
	private double cutoff_frq_percent;
	/**
	 * The starting cutoff frequency for the filter from which filter modulation
	 * can deviate
	 */
	private double initialCutoff;
	/**
	 * The number of poles to allow in the filter. The more poles the faster the
	 * roll_off. 6.0 poles is a good standard to replicate analogue filer
	 * performance.
	 */
	private double poles = 2.0;
	/**
	 * The percent ripple to allow. The more ripple the faster the roll_off. 0.5
	 * percent is a good standard to replicate analogue filter performance
	 */
	 ...
	 * 
	 * @param ao
	 *            The previous Audio Object in the chain.
	 * @param freq
	 ...
	 *            The low pass cutoff frequency.
	 * @param type
	 *            The filter can be LOW_PASS or HIGH_PASS.
	 */
	 ...
	 *            The filter can be LOW_PASS or HIGH_PASS.
	 * @param ripple
	 *            The percentage of ripple to allow. 0.5 is usual.
	 * @param poles
	 *            The greater the number of poles the steeper the cutoff sloap.
	 */
	public Filter(final AudioObject ao, final double freq, final int type, final double ripple, double poles) {
		super(ao, "[Filter]");
		this.type = type;
	...
	/** Constructor to takes two audio objects, the second for cutoff modulation */
	public Filter(final AudioObject[] aoArray, final double freq, final int type, double ripple, double poles) {
	...
	public void coefficientCalc() {
		...
		for (int i = 0; i < 20; i++) {
			...
			if (type == HIGH_PASS) {
				sa = sa + (a[i] * Math.pow(-1.0, i));
			}
			if (type == HIGH_PASS) {
				sb = sb + (b[i] * Math.pow(-1.0, i));
			}
		}
		double gain = sa / (1 - sb);
		for (int i = 0; i < 20; i++) {
	...
	private double[] coefficientCalcSupport(final int p) {
		...
		double RP = -Math.cos(Math.PI / (poles * 2.0) + (p - 1) * Math.PI / poles);
		double IP = Math.sin(Math.PI / (poles * 2.0) + (p - 1) * Math.PI / poles);

		if (ripple != 0.0) {
			double ES = Math.sqrt(Math.pow((100.0 / (100.0 - ripple)), 2) - 1);
			double VX = (1 / poles) * Math.log((1.0 / ES) + Math.sqrt((1 / (ES * ES)) + 1));
			double KX = (1 / poles) * Math.log((1.0 / ES) + Math.sqrt((1 / (ES * ES)) - 1));
	...
	public void setCutOff(final double freq) {
		this.cutoff_frequency = freq;
		if (freq <= 0f) {
			System.err.println("Filter error: You tried to use a " + "cuttoff frequency of " + freq
		...
		if (freq > 0.5 * this.sampleRate) {
			System.err.println("Cutoff frequencies above the Nyquist" + " limit are BAD ;) SampleRate = " + sampleRate
					+ " Frequency = " + freq);
			System.err.println("Exiting from Filter");
			System.exit(1);
		}
	...
	public int work(float[] buffer) throws AOException {
		...
		for (; i < returned; i++) {
			...
			if (i % 100 == 0 && this.previous.length > 1) {
				setCutOff(cutoffs[i] + initialCutoff);
				// System.out.println("Cutoff = " + cutoffs[i] + " Initial = " +
				// initialCutoff);
			}
