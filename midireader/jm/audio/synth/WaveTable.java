...
/**
 * Wavetable lookup creates an efficient means for resampling data into any
 * frequency. It is particularly useful for holding simple wave information such
 * as sinewaves and is often used as an oscillator.<br>
 * This WaveTable implementation can accept either one or two inputs. Two inputs
 * expects that amplitude is the left input and frequency is the right input.
 * One input allows the user to specify whether the input is for amplitude or
 * frequency by setting the aoDestination variable((0)amplitude (1)frequency.<br>
 ...
 * not add more constructors to set amp and frq but leave them as set methods).<br>
 * The frqRatio variable is used to produce a frequency which is a ratio against
 * the current notesFrq (the build methods note.getPitch()) variable setting.<br>
 * It is common to use WaveTables as oscillators and even more common for these
 * oscillators to based on simple wave forms. Simple wave forms in jMusic can be
 * retrieved using static method calls to the Oscillator class.<br>
 * 
...
public class WaveTable extends AudioObject {
	...
	/**
	 * Frequency ratio allows an incoming note's pitch to be adjusted to a fixed
	 * ratio amount
	 */
	...
	/**
	 * This constructor sets the WaveTable to act as a processor object taking
	 * in one input. That input can be either amplitude(0) or frequency(1) and
	 * is defined by the aoDestination variable (int).
	 * 
	 * @param ao
	 *            the one input audio object
	 * @param wavetable
	 *            the wave table data
	 * @param aoDestination
	 *            Is this input amplitude(0) or frequency(1)
	 */
	...
	/**
	 * This constructor sets the WaveTable to act as a processor object taking
	 * in two inputs. Input one is defined as amplitude and input two is defined
	 * as frequency.
	 * 
	 * @param ao
	 *            AudioObject as input
	 * @param waveTable
	 *            the lookup table data
	 * @exception AOException
	 *                thrown when two many inputs are attached
	 */
	...
	/**
	 * This constructor sets this wavetable up as a generator object meaning
	 * that it will pass sample information down the chain based on its wave
	 * table data.<br>
	 * Set WaveTable with some initial values including the sampling rate and
	 * the samples to use for this wave table
	 * 
	 * @param inst
	 *            the parent instrument (usually "this")
	 * @param sampleRate
	 *            the sampling rate
	 * @param waveTable
	 *            the wave table data
	...
	/**
	 * This constructor sets the WaveTable to act as a generator object taking
	 * in one input. That input can be either amplitude(0) or frequency(1) and
	 * is defined by the aoDestination variable (int).
	 * 
	 * @param inst
	 ...
	 *            Is this input amplitude(0) or frequency(1)
	 * @param val
	 *            is used to set a fixed frequency or amplitude based on the
	 *            result of aoDestination (aoDestination=1 for example will set
	 *            a fixed frequency)
	 */
	 ...
	 * 
	 * @param frqRatio
	 *            Fixed ratio value to change frequency by
	 */
	...
	// ------------------------------------------
	/**
	 * Returns the sampling increment which is used to nextWork out how many
	 * samples in the wavetable skip on each pass.
	 * 
	 * @param frequency
	 *            the frequency used to find si
	 */
	protected void setSI(float frequency) {
		this.si = (int) ((frequency / this.sampleRate) * this.waveTable.length);
	}
	 ...
	 * nextWork method can take one or two inputs which are either amplitude,
	 * frequency or both (a single input can be assigned to either frequency or
	 * amplitude by assigning the aoDestination value to either (0)Amp or (1)Frq
	 * in the appropriate constructor. A WaveTable that takes two inputs expects
	 * the first input to be amplitude and the second input to be frequency.
	 * 
	 * @param buffer
	 *            The sample buffer.
	 */
			...
			for (; ret < buffer.length;) {
				...
				for (int j = 0; j < channels; j++) {
					try {
						...
						//
						// We do need to remove one back off ret though to
						// return
