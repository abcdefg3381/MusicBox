...
public final class Pluck extends AudioObject {
	...
	/**
	 * This constructor takes a single AudioObject as input and in this becomes
	 * a processor object which filters it's input through the Karpulus-Strong
	 * algorithm.
	 * 
	 * @param ao
	 ...
	 *            the single AudioObject to use as input.
	 * @param feedback
	 *            The sustain level (0.5 is maximum, changes as small as 0.001
	 *            are significant)
	 */
	 ...
	 * 
	 * @param inst
	 *            The containing instrument, usually passed 'this'
	 * @param sampleRate
	 *            The integer number of samples per second (higher rates produce
	 *            brighter timbre).
	 * @param channels
	 *            The interger number of channels, 1 = mono, 2 = stereo, etc.
	 */
	...
	public void build() {
		...
		int length = (int) (sampleRate / freq);
		this.kernel = new float[length];
		for (int i = 0; i < length; i++) {
			if (primary) {
				kernel[i] = (float) (Math.random() * 2.0 - 1.0); // start with
																	// noise
			} else {
	 ...
	 * 
	 * @param feedback
	 *            New interation feedback value, equals the sustain level (0.5
	 *            is maximum, changes as small as 0.001 are significant)
	 */
	...
	/**
	 * Create or process samples through the filter. Called for each buffer of
	 * smaples.
	 */
	@Override
			...
			for (; i < buffer.length;) {
				...
				for (int j = 0; j < channels; j++) {
					...
					try {
						...
						System.out.println("jMusic Pluck audio object error: i = " + i + " delayIndex = " + delayIndex);
					}
