 ...
 * 
 * @author Timothy Opie
 * @version 1.0, Wednesday November 12, 2003 Last changed: Description: This
 *          class takes a stream of audio data and reduces it to grain
 *          information comprising solely of a start and finish time band, a top
 *          and bottom frequency band and an amplitude.
 */

public final class AllFGTR extends AudioObject {
	private float /* grainDuration, */bandwidthTop, bandwidthBottom;
	private int bCounter = 0, gCounter = 0, dCounter = 0;// This counter keeps
	// ----------------------------------------------
	// Attributes
	// ----------------------------------------------
	// private float[] grain = new float[buffer.length]; //grain buffer
	private float[][] FGTArray = new float[200][7];
	private float frequency, highestAmp = 0.0f;
	// track of the
	// buffer
	...
	public AllFGTR(AudioObject ao, int gDuration, float bwidthTop, float bwidthBottom, int gps) {
		...
		this.bandwidthTop = bwidthTop; // this filter info is purely for data
										// collection puroposes
	...
	public int work(float[] buffer) throws AOException {
		...
		// System.out.println("grainsPerSecond: " + grainsPerSecond);
		// Regarding the filter: I am thinking it will be easier to initiate the
		// filter
		// as part of the instrument, rather than in this section, so that
		// FGT.java will
		// assume it is using a filtered audio buffer. This may change in the
		// future.....
		// Filter filterOne = new Filter(buffer, bandwidthBottom,
		// Filter.LOW_PASS);
		// Filter filterDone = new Filter(filterOne, bandwidthTop,
		// Filter.HIGH_PASS);
		highestAmp = 0.0f;
		bCounter = 0;
		gCounter = 0;
		...
		// System.out.println("grainsPerSecond: " + grainsPerSecond +
		// "buffer.length: " + buffer.length + "sampleRate: " + sampleRate +
		// "channels: " + channels + "grainsPerBuffer: " + grainsPerBuffer);
		...
		for (gCounter = 0; gCounter < grainsPerBuffer; gCounter++) {
			...
			for (dCounter = 0; dCounter < grainDuration; dCounter++) {
				...
				// System.out.println("dCounter/grainDuration: " +
				// (float)dCounter/(float)grainDuration);
				...
				// System.out.println("Amp:" + temp2 + "Highest:" + highestAmp);
				// //just for bug checking
				bCounter++;
			}
			...
			FGTArray[gCounter][4] = 0.5f; // Stereo placement - will be improved
											// later!
			FGTArray[gCounter][5] = highestAmp;
			FGTArray[gCounter][6] = grainsPerBuffer;

			// System.out.println("gCounter: " + gCounter + "bCounter: " +
			// bCounter);
			// System.out.println("gCounter*interOnset2: " +
			// gCounter*interOnset);
		...
		for (gCounter = 0; gCounter < grainsPerBuffer; gCounter++) {
			...
			frequency = (bandwidthTop + bandwidthBottom) * 0.5f; // to be
																	// improved
																	// later
