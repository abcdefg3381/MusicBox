...
/**
 * The Oscillator class can generate steady tones with various wave shapes,
 * inlcuding sine, cosine, triangle, sawtooth, pulse wave, square, and more. The
 * Oscillator class can be used as a primary object at the start of an audio
 * chain, or as a object within the chain who's frequency or amplitude is
 * modulatoed by an earlier object.
 * 
...
public class Oscillator extends AudioObject {
	...
	/**
	 * The constant that represents the sine wave form. The pulse width can be
	 * set using setPulseWidth() method.
	 */
	...
	/**
	 * This constructor sets the Oscillator to act as a processor object taking
	 * in one input. That input can be either amplitude(0) or frequency(1) and
	 * is defined by the choice variable (int).
	 * 
	 * @param ao
	 *            the one input audio object
	 * @param waveType
	 *            the type of timbre to generate
	 * @param choice
	...
	/**
	 * This constructor sets this Oscillator up as a generator using default
	 * parameters
	 * 
	 * @param Instrument
	 *            the instance this is associated with 'this'
	 */
	...
	/**
	 * This constructor sets this Oscillator up as a generator specifying the
	 * type of waveform to use
	 * 
	 * @param Instrument
	 *            the instance this is associated with 'this'
	 * @param waveType
	 *            an integer or constant sepcifying the noise type
	 */
	...
	/**
	 * This constructor sets this Oscillator up as a generator specifying the
	 * type of waveform and sample rate to use
	 * 
	 * @param Instrument
	 *            the instance this is associated with 'this'
	 * @param waveType
	 *            an integer or constant sepcifying the noise type
	 * @param sampleRate
	 *            an int that sets the sample rate in samples per second
	 */
	...
	/**
	 * This constructor sets this Oscillator up as a generator specifying the
	 * type of wavetable and ferquency
	 * 
	 * @param Instrument
	 *            the instance this is associated with 'this'
	 * @param waveType
	...
	protected float getWaveSample() {
		...
		case SABERSAW_WAVE:
			...
			// System.out.println("Position = " + position + " Sample = " +
			// sample);
			phase += si;
			if (phase >= (2.0f * (float) Math.PI)) {
				phase -= 2.0f * (float) Math.PI;
	 ...
	 * 
	 * @param choiceVal
	 *            0 = fixed amplitude, 1 = fixed frequency
	 */
	 ...
	 * 
	 * @param phase
	 *            The phase in radians (between 0.0 and 2 * PI)
	 */
	 ...
	 * samples in the Oscillator skip on each pass.
	 * 
	 * @param frequency
	...
	protected void setSI(double frequency) {
		...
		/*
		 * if(frequency <= 0f) { System.err.println(
		 * "Oscillator error: You tried to use a frequency less than zero - woops!"
		 * ); System.exit(1); }
		 */
		this.si = 2.0f * (float) Math.PI / (this.sampleRate / (float) frequency);
	}
	 ...
	 * amplitude by assigning the choice value to either (0)Amp or (1)Frq in the
