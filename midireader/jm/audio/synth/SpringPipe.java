...
public class SpringPipe {
	...
	private void updateSpringMassNetwork() {
		...
		for (int i = 1; i < massObjectArray.length; i++) {
			forceArray[i] =
					springObjectArray[i].getCurrentForce(massObjectArray[i - 1].getYPosition() + width,
							massObjectArray[i].getYPosition());
		}
		// final spring
		forceArray[forceArray.length - 1] =
				springObjectArray[forceArray.length - 1].getCurrentForce(
						massObjectArray[massObjectArray.length - 1].getYPosition() + width, totalLength);
