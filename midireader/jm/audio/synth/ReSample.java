...
public final class ReSample extends AudioObject {
	...
	public void build() {
		if (noteFreq) {
			this.newFreq = FRQ[this.currentNote.getPitch()];
		}
		this.finished = true;
	}
	...
	// ----------------------------------------------
	/**
	 * This is nextWork method simply needs to skip over the right number of
	 * samples to cause a shorter wave period. This means that the pitch goes
	 * up. The number of samples to skip is calculated by the setPhaseInt()
	 * method which is the real nextWork horse of this class.
	 * 
	 * @param input
	 *            a single Audio Object input.
	 */
	@Override
	public int work(float[] buffer) throws AOException {
		...
		float[] tmpBuf = new float[(int) (buffer.length * cfm + 0.5) + 1];
