 ...
 * @version 1.0,Sun Feb 25 18:42:48 2001 Revised and modified extensively by
 *          Timothy Opie Last changed v1.4 27/04/2003
 */
...
public final class Granulator extends AudioObject {
	...
	public Granulator(AudioObject ao, int sampleRate, int channels, float[] durationArr, float[] gpsArr, float[] freqArr,
			boolean rif, boolean rgdf, boolean rff, int rd) {
		super(ao, "[Granulator]");
	...
	private void setGrain(int index) throws AOException {
		if (ri) {
			index = (int) (Math.random() * newbuf.length);
		}
		...
		if (rgd) {
			...
			/*
			 * { if (premapped) { this.cgd = (int)durationArray[grainCnt]; }
			 * else {
			 */
		...
		if (envelopeType <= 1) {
			for (int i = 0; i < cgd; i++) {
				this.grain[i] = this.grain[i] * (float) (0.5 - 0.5 * Math.cos(2 * Math.PI * i / cgd));
			}
		}
		...
		// }
			...
			for (int i = 0; i < cgd; i++) {
				this.grain[i] = this.grain[i] * (float) (Math.sin(Math.PI * i / cgd));
	...
	public int work(float[] buffer) throws AOException {
		...
		// } else {
		nog = (int) (newbuf.length / ((float) (sampleRate * channels) / (float) grainsPerSecond));
		// }
		if (nog <= 0) {
			nog = 1;
		}
		...
		for (int i = 0; i < nog; i++) {
			...
			for (float element : grain) {
				if (index >= buffer.length) {
					tailBuf[index - buffer.length] += element;
				} else {
