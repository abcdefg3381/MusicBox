...
/**
 * An Instrument in jMusic is a chain of AudioObjects which are strung together
 * to form a signal processing chain. Instruments chains are each run in their
 * own thread.
 * 
...
public abstract class Instrument extends Thread implements Runnable, jm.JMC {
	 ...
	 * playback. This normally means the SampleOut audio object is NOT part of
	 * the audio chain.
	 */
	public final static int REALTIME = 1;
	/**
	 * Output from instrument to go to a file of the hard drive. This is
	 * normally associated with using the SampleOut audio object
	 */
	 ...
	 * NOTE: This may be longer than both the note's duration and the note's
	 * rhythmvalue !
	 */
	...
	/**
	 * Real time export Buffer which is ALWAYS bufsize (i.e., not changed to
	 * reflect a change in the notes duration which is why we need an
	 * rtBufferIndex).
	 */
	...
	/**
	 * block is called to hold any further processing untill all instruments are
	 * in the same place (in time that is). Block calls wait on this instrument
	 * and is released by a call to release. The RTAudio class is responsible
	 * for handling all block and release calls.
	 */
	...
	// ----------------------------------------------
	/**
	 * This method is automatically called on startup to initialise any
	 * AudioObjects used by this instrument
	 */
	...
	/**
	 * Check on the current output flag status. Normally 0 = render to file, 1 =
	 * play via realtime output. However, these flags must be handled by each
	 * instrument, as they are not global.
	 */
	...
	// ----------------------------------------------
	/**
	 * The iterate chain method is resposible for pulling the appropriate number
	 * of samples through the audio chain.
	 */
	...
	/**
	 * release is called by RTAudio when the RTLine is ready to start processing
	 * data again. This is required so that all instruments in turn provide the
	 * buffer they are currently processing to the real time audio stream
	 */
	 ...
	 * render a new note. This method is responsible for caclulating the number
	 * of samples required for the note and calls any AudioObjects build methods
	 * prior to starting the chain.
	 * 
	 * @param note
	 *            The note to render
	 * @param startTime
	...
	public void renderNote(Note note, double startTime) {
		...
		// continue onward
		this.numOfSamples = (int) (pao.getSampleRate() * ((float) note.getDuration()));
		...
		if (note.getFrequency() == REST) {
			restNote = true;
		} else {
			// calc frequency
			double frequency = 0.0;
			if (note.getPitchType() == Note.MIDI_PITCH && note.getPitch() != Pitches.REST) {
				frequency = Frequencies.FRQ[note.getPitch()];
			} else {
				frequency = note.getFrequency();
			}
			if (pao.getSampleRate() * 0.5 < frequency) {
	...
	/**
	 * Sets the buffer size which is used to pass sample data between work
	 * methods.
	 * 
	 * @param bufsize
	 *            The buffer size to set for this instrument
	 */
	 ...
	 * method. Controller values which are not doubles will need to be cast to
	 * their appropriate types in the overriding method.
	 * 
	 * @param controlValues
	 */
	...
	public void setFinalAO(AudioObject ao) throws AOException {
		if (finalAO == null || finalAO == ao) {
			...
			throw new AOException("jMusic Instrument error: " + ao.name, finalAO.name + " is already set as finalAO.\n"
					+ "  There can only be one finalAO.");
		}
	}

	/**
	 * Is the instrument still processing a note? If false the note will end
	 * abruptly.
	 */
	...
	/**
	 * Select the output flage value. This is normallyused to specify if the
	 * SampleOut object will be included in the audio chain. There are constats
	 * in this class for common values, RENDER = 0 and REALTIME = 1.
	 */
