...
/**
 * The <bl>SampleOut</bl> class is responsible for writing sample data to disk.
 * It combines all notes into a single Random Access File based on each notes
 * start time. The output of samples into this file is in 32 bit floating point
 * format and a maximum amplitude for this file is calculated on the fly and
 * storied by this class for easy normalisation when being rewritten to a
 * standard Audio File Format.<br>
 * <br>
 * NOTE: One important thing to note about the class is that all the notes of
 * the score are written to the same file. Therefore everytime this class is
 * called for each new note the information is continually written to the same
 * temporary file often combining new data with existing data.<br>
 * <br>
 * The SampleOut class only excepts a Single Audio Object as input and should
 * therefore have all Audio Streams combined before being passed to the
 * <bl>SampleOut</bl>.
 * 
...
public final class SampleOut extends AudioObject implements JMC {
	 ...
	 * 
	 * @param ao
	 *            a single Audio Object for input.
	 */
	...
	/**
	 * Excepts a single Audio Object only. The filename will be the name used to
	 * write the temporary combined floating point data to disk. This file
	 * should be the same for all instances of this object.
	 * 
	 * @param ao
	 *            a single Audio Object for input.
	 * @param fileName
	 *            to use for all data written to disk.
	 */
	 ...
	 *            to use for all data written to disk.
	 * @param sync
	 *            do we want to sync all data to disk?
	 */
	 ...
	 *            to use for all data written to disk.
	 * @param position
	 *            shows where in the RAF to write to (in bytes).
	 * @param sync
	 *            do we want to sync all data to disk?
	 */
	...
	// ----------------------------------------------
	/**
	 * set the position to write to in the RAF for this note based on the notes
	 * startTime.
	 * 
	 * @param note
	 *            the note that this objects Audio Chain is currently rendering
	 * @param startTime
	 *            a notes startTime which is changed into a RAF position.
	 */
	@Override
	public void build() {
		...
		this.position = (int) (currentNoteStartTime * this.sampleRate) * 4 * channels;
		if (this.position < 0) {
			this.position = 0;
		}
	...
	public void finalize() {
		/*
		 * try{ this.raf.close(); }catch(IOException ioe){
		 * ioe.printStackTrace(); }
		 */
	}
	...
	// ----------------------------------------------
	/**
	 * Write any incoming samples out to disk using the position (which is set
	 * by the note and then moved appropriately for each sample written) to
	 * place the sample into the RAF.<br>
	 * NOTE: Remember that only one AudioObject can ever be an input to this
	 * Object.
	 */
	@Override
	...
	// ----------------------------------------------
	/**
	 * The write method is used to write sample information from the buffer
	 * (bos) to the RAF based on the samples current position withing the
	 * jm.Score. All of this is handled by this class in general. Sorry about
	 * the mess that is this class.
	 */
