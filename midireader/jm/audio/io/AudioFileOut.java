...
/**
 * Audio file writing class for jMusic. The class utilises the JavaSound file
 * writing classes. It works indepdently of the jMusic audio architecture used
 * for rendering scores, and is intended for use by simple file manipulation
 * utility programs operating asychronously (non real time).
 * 
 * @author Andrew Brown
 */
...
public class AudioFileOut {
	...
	public AudioFileOut(float[] sampleData, String fileName, int channels, int sampleRate, int sampleSizeInBits) {
		...
		for (int i = 0; i < sampleData.length; i++) {
			...
			switch (sampleSize) {
			case 1: // 8 bit
				tmp[i] = new Float(sampleData[i] * Byte.MAX_VALUE).byteValue();
				break;
			case 2: // 16 bit
				short sval = new Float(sampleData[i] * Short.MAX_VALUE).shortValue();
				if (bigEndian) {
				...
				break;
			case 3: // 24 bit
				ival = new Float(sampleData[i] * 8388608).intValue();
				if (bigEndian) {
				...
				break;
			case 4: // 32 bit
				ival = new Float(sampleData[i] * Integer.MAX_VALUE).intValue();
				if (bigEndian) {
