...
/**
 * Audio file reading class for jMusic. The class utilises the JavaSound file
 * reading classes. It works indepdently of the jMusic audio architecture used
 * for rendering scores, and is intended for use by simple file manipulation
 * utility programs operating asychronously (non real time). This class deals
 * with some the current (at time of writing) incompletness in the javaSound
 * package, including converting bytes from a file into floats for both big and
 * little endian formats, and support for 24 bit files.
 * 
...
public class AudioFileIn {
	 ...
	 * 
	 * @param fileName
	 *            The name of the file to read, e.g., filename.aif
	 */
	...
	private void readFile() {
		if (audioFileSpecified) {
			...
			try {
				...
				int val = fis.read();
				while (val != -1) {
					buffer.addElement(new Float(val / 255f));
					val = fis.read();
				}
				fis.close();
			} catch (IOException ioe) {
