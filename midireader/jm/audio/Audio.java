...
/**
 * The Audio class provides a number of static methods to help pass a jmusic
 * score to the audio architecture and for putting a notes sample information
 * into the correct location in a global audio file.<br>
 * <br>
 * WARNING !!!!!!! This class is an absolute disgrace ;) It works but is very
 * ugly and I can't be bothered to clean it up at the moment. If anyone feels
 * like cleaning it up go for it ;)
 * 
...
public final class Audio implements jm.JMC {
			...
			try {
				for (;;) {
					...
					if (outgoing < (float) -1.0 || outgoing > (float) 1.0) {
						System.out.println("Outgoing= " + outgoing + "  SAMPLE: " + samp + "  MAX: " + max
			...
			try {
				...
				for (;;) {
					try {
						 ...
						 * File fl = new File(fileName); fl.delete(); }
						 */
						if ((numofdot % 10) == 0) {
							if (VERBOSE) {
		...
		for (;;) {
			try {
				...
				try {// if we can read from the file
					...
					// This means that there is nothing to read so we can
					// happily write
					if (max < sample) {
						max = sample;
					}
	...
	/**
	 * Makes an array which contains all the notes from all the phrases from all
	 * the instruments etc. solely based on start times. This method also writes
	 * a jpf file which is a text file containing the names of all the notes
	 * corresponding audio files with their start times and lengths.
	 * 
	 * @param score
	 *            the score to take data from
	 */
	public static void processScore(Score score, Instrument[] instList, String fileName) {
		Stack inst = new Stack();
		// add an instrument to avoid errors from no instrument assignment by
		// user
		...
		while (enum1.hasMoreElements()) {
			...
			if (part.getInstrument() != NO_INSTRUMENT) {
				try {
					...
					System.out.println("jMusic Audio warning: Can't find the instrument number " + part.getInstrument()
							+ " that you have specified for " + "the part named " + part.getTitle() + ".");
				}
			}
			...
			while (enum2.hasMoreElements()) {
				...
				if (phr.getInstrument() != NO_INSTRUMENT) {
					try {
						...
						System.out.println("jMusic Audio warning: Can't find the instrument number " + phr.getInstrument()
								+ " that you have specified for" + " the phrase named " + phr.getTitle() + ".");
					}
				}
				double time = part_ratio * phr.getStartTime(); // start time of
																// phrase
				double ntime = 0.0; // notes distance from phrases start time
				Enumeration enum3 = phr.getNoteList().elements();
				System.out.print("    Phrase " + phraseCounter++ + " '" + phr.getTitle() + "'" + " starting at beat "
						+ phr.getStartTime() + ": ");
				...
				while (enum3.hasMoreElements()) {
					...
					if (note.getFrequency() == REST) { // This a rest
														// ???
					...
					// System.out.println("new note pitch = " +
					// new_note.getPitch());
